...
ldap-search: ## Search LDAP server (all entries)
	...
	@docker exec flext-openldap-test ldapsearch -x -H ldap://localhost:389 -D "cn=REDACTED_LDAP_BIND_PASSWORD,dc=flext,dc=local" -w "REDACTED_LDAP_BIND_PASSWORD123" -b "dc=flext,dc=local"

.PHONY: ldap-search-users
ldap-search-users: ## Search LDAP users
	@echo "🔍 Searching LDAP users..."
	@docker exec flext-openldap-test ldapsearch -x -H ldap://localhost:389 -D "cn=REDACTED_LDAP_BIND_PASSWORD,dc=flext,dc=local" -w "REDACTED_LDAP_BIND_PASSWORD123" -b "ou=users,dc=flext,dc=local"

.PHONY: ldap-search-groups
ldap-search-groups: ## Search LDAP groups
	@echo "🔍 Searching LDAP groups..."
	@docker exec flext-openldap-test ldapsearch -x -H ldap://localhost:389 -D "cn=REDACTED_LDAP_BIND_PASSWORD,dc=flext,dc=local" -w "REDACTED_LDAP_BIND_PASSWORD123" -b "ou=groups,dc=flext,dc=local"
