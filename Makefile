...
ldap-test: ## Test LDAP connection
	$(POETRY) run python -c "from flext_ldap import FlextLdapClient; print('LDAP test passed')"
...
.DEFAULT_GOAL := help
.PHONY: help install install-dev setup validate check lint format type-check security fix test test-unit test-integration test-ldap test-fast coverage-html ldap-test build build-clean docs docs-serve deps-update deps-show deps-audit shell pre-commit clean clean-all reset diagnose doctor t l f tc c i v