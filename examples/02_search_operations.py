...
def demonstrate_search_with_request(api: FlextLdap) -> None:
    ...
    logger.info(f"✅ Found {len(response.entries)} organizational units")
    for i, entry in enumerate(response.entries, 1):
        ou = entry.attributes.get("ou", ["N/A"])
...
def demonstrate_group_search(api: FlextLdap) -> None:
    ...
    groups: list[FlextLdapModels.Entry] = result.unwrap()
    logger.info(f"✅ Found {len(groups)} groups")
    for i, group in enumerate(groups[:3], 1):  # Show first 3
        logger.info(f"   {i}. Group DN: {group.dn}")
        # Access attributes through the attributes dictionary
        cn_value = group.attributes.get("cn", "N/A")
        member_dns = group.attributes.get("member", [])
...
def demonstrate_filter_validation(_api: FlextLdap) -> None:
    ...
    test_filters = [
        ...
        ("(|(cn=REDACTED_LDAP_BIND_PASSWORD)(cn=user))", True, "Valid OR filter"),
...
def demonstrate_dn_validation() -> None:
    ...
    # Test various DNs
    test_dns = [
        ("cn=REDACTED_LDAP_BIND_PASSWORD,dc=example,dc=com", True, "Valid standard DN"),
...
def demonstrate_attribute_filtering(api: FlextLdap) -> None:
    ...
    if entries:
        ...
        entry = entries[0]
        logger.info(f"   First entry attributes: {list(entry.attributes.data.keys())}")
    else:
