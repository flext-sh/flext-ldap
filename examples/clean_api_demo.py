...
async def demo_clean_api() -> None:
    ...
    async with LDAP(config) as ldap:
        ...
        await ldap.search().groups("REDACTED_LDAP_BIND_PASSWORD*")
        ...
        await ldap.groups().find_by_name("REDACTED_LDAP_BIND_PASSWORDs")

        # Obter membros do grupo
        await ldap.groups().get_members("cn=REDACTED_LDAP_BIND_PASSWORDs,dc=example,dc=com")
...
"""
        ...
        capabilities = await ldap.REDACTED_LDAP_BIND_PASSWORD().get_server_capabilities()
        if capabilities.success:
            pass

        # Obter Root DSE
        await ldap.REDACTED_LDAP_BIND_PASSWORD().get_root_dse()
