...
Enterprise LDAP operations library. Base for the LDAP Singer connectors
...
- **Use the `FlextLdap` facade** — do not reach into `adapters/` or `services/`
  internals directly.
- LDIF parsing/serialization is delegated to `flext-ldif` (registry-selected dialects),
