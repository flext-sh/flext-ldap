...
# Test connection
ldapsearch -x -H ldap://localhost -b dc=test,dc=com -D cn=REDACTED_LDAP_BIND_PASSWORD,dc=test,dc=com -w REDACTED_LDAP_BIND_PASSWORD123
```
...
docker run -p 3890:3890 -p 17170:17170 \
  ...
  -e LLDAP_LDAP_USER_PASS="REDACTED_LDAP_BIND_PASSWORD123" \
  lldap/lldap:stable

# Access web UI at http://localhost:17170
# Username: REDACTED_LDAP_BIND_PASSWORD, Password: REDACTED_LDAP_BIND_PASSWORD123
```
...
Port: 389
Bind DN: cn=read-only-REDACTED_LDAP_BIND_PASSWORD,dc=example,dc=com
Password: password
...
conn = Connection(server, 'cn=read-only-REDACTED_LDAP_BIND_PASSWORD,dc=example,dc=com', 'password')
...
conn = Connection(server, 'cn=REDACTED_LDAP_BIND_PASSWORD,dc=test,dc=com', 'REDACTED_LDAP_BIND_PASSWORD123')
...
public class LdapQuickStart {
    public static void main(String[] args) throws LdapException {
        ...
        connection.bind("cn=read-only-REDACTED_LDAP_BIND_PASSWORD,dc=example,dc=com", "password");
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    ldap.simple_bind("cn=read-only-REDACTED_LDAP_BIND_PASSWORD,dc=example,dc=com", "password")?;
...
ldap = Net::LDAP.new(
  ...
  auth: {
    method: :simple,
    username: 'cn=read-only-REDACTED_LDAP_BIND_PASSWORD,dc=example,dc=com',
    password: 'password'
  }
)
...
func main() {
    ...
    // Bind
    err = l.Bind("cn=read-only-REDACTED_LDAP_BIND_PASSWORD,dc=example,dc=com", "password")
    if err != nil {
        log.Fatal(err)
    }
...
class Program
{
    static void Main()
    {
        ...
        using (var entry = new DirectoryEntry("LDAP://ldap.forumsys.com/dc=example,dc=com",
                                              "cn=read-only-REDACTED_LDAP_BIND_PASSWORD,dc=example,dc=com",
                                              "password"))
        {
...
// Bind
$bind = ldap_bind($ldap, "cn=read-only-REDACTED_LDAP_BIND_PASSWORD,dc=example,dc=com", "password");
...
- **📖 [phpLDAPREDACTED_LDAP_BIND_PASSWORD](reference/phpldapREDACTED_LDAP_BIND_PASSWORD-web-interface/)** - Complete web REDACTED_LDAP_BIND_PASSWORD
...
- **[🖥️ Try GUI Tools](reference/README.md#gui--REDACTED_LDAP_BIND_PASSWORDistration-tools)** - Visual REDACTED_LDAP_BIND_PASSWORDistration
