...
user_analytics_enriched AS (
    SELECT
        ...
        CASE
            WHEN group_memberships ILIKE '%REDACTED_LDAP_BIND_PASSWORD%' THEN TRUE
            ELSE FALSE
        END as has_REDACTED_LDAP_BIND_PASSWORD_privileges,
...
department_analytics AS (
    SELECT
        ...
        COUNT(CASE WHEN has_REDACTED_LDAP_BIND_PASSWORD_privileges THEN 1 END) as users_with_REDACTED_LDAP_BIND_PASSWORD_access,
...
{% macro analyze_ldap_security_posture() %}
    ...
    SELECT
        ...
        COUNT(CASE WHEN has_REDACTED_LDAP_BIND_PASSWORD_privileges AND last_login_date < CURRENT_DATE - INTERVAL '30 days' THEN 1 END) as dormant_REDACTED_LDAP_BIND_PASSWORD_accounts,
...
class FlextEnterpriseLDAPAuthProvider(AuthProvider):
    ...
    def _extract_enterprise_roles(self, group_memberships: list[str]) -> list[str]:
        ...
        roles = []
        role_mappings = {
            "cn=enterprise-REDACTED_LDAP_BIND_PASSWORDs": "enterprise_REDACTED_LDAP_BIND_PASSWORD",
