...
async def group_operations_example():
    ...
    async with LDAP(config) as ldap:
        ...
        if is_member.success:
            print(f"Is john.doe admin? {is_member.data}")
...
async def session_context_example():
    ...
    async with ldap_session(
        ...
        user = await ldap.find_user_by_email("admin@company.com")
...
*Documentação criada em: 2025-06-26*  
*Refatoração: God Object → True Facade Pattern*  
*Status: 100% compatível + enhanced capabilities*