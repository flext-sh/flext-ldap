...
- op timeout handling:
  - mark for abandon
  - send abandon
  - send timeLimitExceeded/REDACTED_LDAP_BIND_PASSWORDLimitExceeded to client
...
- default option is not to care and if a control/exop has special restrictions,
  it is the REDACTED_LDAP_BIND_PASSWORD's job to flag it as such in the load-balancer's config
