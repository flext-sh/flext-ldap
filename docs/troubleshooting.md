...
ldapsearch -x -H ldap://ldap.example.com:389 -D "cn=admin,dc=example,dc=com" -w password -b "dc=example,dc=com"
```
...
ldapsearch -x -H ldap://ldap.example.com:389 -ZZ -D "cn=admin,dc=example,dc=com" -w password
```
...
4. **Test with LDAP admin tools** first
...
```python
ImportError: cannot import name 'FlextLdapClient' from 'flext_ldap'
```
...
ldapsearch -v -x -H ldap://server:389 -D "cn=admin,dc=example,dc=com" -w password
ldapwhoami -v -x -H ldap://server:389 -D "cn=admin,dc=example,dc=com" -w password
```
