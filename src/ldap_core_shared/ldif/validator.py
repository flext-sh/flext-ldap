...
class LDIFValidationConfig(BaseModel):
    ...
    validate_attribute_syntax: bool = Field(default=True, description="Validate attribute syntax")
    validate_object_classes: bool = Field(default=True, description="Validate object class consistency")
    validate_required_attributes: bool = Field(default=True, description="Check required attributes")
    allow_binary_attributes: bool = Field(default=True, description="Allow binary attribute values")
    max_dn_length: int = Field(default=8192, ge=1, description="Maximum DN length")
    max_attribute_length: int = Field(default=65536, ge=1, description="Maximum attribute value length")
...
class LDIFValidator:
    ...
    def validate_entries(self, entries: list[LDIFEntry]) -> LDAPValidationResult:
        ...
        for i, entry in enumerate(entries):
            ...
            warnings.extend([f"Entry {i} ({entry.dn}): {w}" for w in entry_result.warnings])
    ...
    def _validate_dn(self, dn: str) -> list[str]:
        ...
        if len(dn) > self.config.max_dn_length:
            errors.append(f"DN exceeds maximum length: {len(dn)} > {self.config.max_dn_length}")
    ...
    def _validate_attributes(self, attributes: dict[str, list[str]]) -> tuple[list[str], list[str]]:
            ...
            for value in attr_values:
                ...
                if not self.config.allow_binary_attributes:
                    try:
                        ...
                        errors.append(f"Binary content not allowed in attribute '{attr_name}'")
