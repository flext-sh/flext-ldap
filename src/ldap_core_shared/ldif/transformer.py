...
class AttributeTransformRule(TransformationRule):
    ...
    target_attribute: str | None = Field(default=None, description="Target attribute name (None = same as source)")
    transformation_type: str = Field(..., description="Type of transformation")
    transformation_params: dict[str, Any] = Field(default_factory=dict, description="Transformation parameters")
...
class EntryFilterRule(TransformationRule):
    ...
    filter_params: dict[str, Any] = Field(default_factory=dict, description="Filter parameters")
    include_matches: bool = Field(default=True, description="Include matching entries (True) or exclude them (False)")
...
class LDIFTransformer:
    ...
    def transform_entries(self, entries: list[LDIFEntry]) -> LDAPOperationResult[list[LDIFEntry]]:
            ...
            for attr_key in list(new_attributes.keys()):
                if attr_key.lower() == attribute_name.lower():
                    try:
                        ...
                        logger.warning(f"Failed to transform attribute {attr_key} in {entry.dn}: {e}")
            
            transformed.append(LDIFEntry(
                dn=entry.dn,
    ...
    def _apply_filter_rule(self, entry: LDIFEntry, rule: EntryFilterRule) -> bool | None:
        ...
        try:
            ...
            elif rule.filter_type == "object_class":
    ...
    def _transform_entry_attributes(self, entry: LDIFEntry) -> LDIFEntry:
        ...
        for rule in self._attribute_rules:
            ...
            try:
                ...
                logger.warning(f"Failed to apply attribute rule {rule.name} to {entry.dn}: {e}")
