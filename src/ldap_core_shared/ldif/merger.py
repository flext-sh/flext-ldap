...
from .writer import LDIFHeaderConfig, LDIFWriter
...
class MergeConfig(BaseModel):
    ...
    conflict_resolution: str = Field(default="first_wins", description="How to resolve conflicts")
...
class LDIFMerger:
    ...
    def _merge_entries(self, entries: list[LDIFEntry], stats: MergeStatistics) -> list[LDIFEntry]:
    ...
    def _resolve_conflict(self, entries: list[LDIFEntry]) -> LDIFEntry:
        ...
        if self.config.conflict_resolution == "first_wins":
            return entries[0]
        elif self.config.conflict_resolution == "last_wins":
            return entries[-1]
        elif self.config.conflict_resolution == "merge_attributes":
            return self._merge_attributes(entries)
        elif self.config.conflict_resolution == "most_complete":
