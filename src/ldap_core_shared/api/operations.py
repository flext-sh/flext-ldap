...
class LDAPOperations:
    ...
    def __init__(self, config: LDAPConfig, connection_manager: ConnectionManager | None = None) -> None:
        ...
        """
        self._config = config
        self._connection_manager = connection_manager or self._create_connection_manager(config)
    ...
    def _create_connection_manager(self, config: LDAPConfig) -> ConnectionManager:
        ...
        from ldap_core_shared.connections.manager import create_unified_connection_manager
    ...
    async def find_user_by_email(self, email: str) -> Result[LDAPEntry | None]:
        ...
        try:
            ...
            base_dn = self._config.base_dn or "dc=example,dc=com"
            filter_expr = f"(mail={email})"
            ...
            return Result.ok(None, message=f"User search for {email} delegated to core operations")
    ...
    async def find_users_in_department(self, department: str) -> Result[list[LDAPEntry]]:
        ...
        from ldap_core_shared.api.results import Result
        try:
            logger.info(f"Finding users in department: {department}")
            # Pure delegation to core operations would go here
            return Result.ok([], message=f"Department search for {department} delegated to core operations")
        except Exception as e:
    ...
    async def test_connection(self) -> Result[bool]:
        ...
        try:
            ...
            return Result.ok(True, message="Connection test delegated to connection manager")
        except Exception as e:
