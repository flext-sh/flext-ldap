...
class SchemaMigrator:
    ...
    def _generate_addition_ldif_content(
        ...
        # Header
        lines.extend(("# Schema Additions LDIF", "# Generated by ldap-core-shared Schema Migrator", "", "version: 1", ""))

        # Add attribute types
        if attr_additions:
            lines.extend(("# Add Attribute Types", "dn: cn=schema", "changetype: modify", "add: attributeTypes"))

            lines.extend(f"attributeTypes: {diff.target_definition}" for diff in attr_additions if diff.target_definition)

            lines.extend(("-", ""))

        # Add object classes
        if oc_additions:
            lines.extend(("# Add Object Classes", "dn: cn=schema", "changetype: modify", "add: objectClasses"))

            lines.extend(f"objectClasses: {diff.target_definition}" for diff in oc_additions if diff.target_definition)
    ...
    def _generate_modification_ldif_content(
        ...
        # Header
        lines.extend(("# Schema Modifications LDIF", "# WARNING: Review carefully before applying", "", "version: 1", ""))
        ...
        if attr_mods:
            ...
            for diff in attr_mods:
                lines.extend((f"# Element: {diff.element_name}", f"# Change: {diff.description}", f"# From: {diff.source_definition}", f"# To: {diff.target_definition}", "#", "# Manual modification required", ""))
    ...
    def _generate_removal_ldif_content(self, removals: list[SchemaDifference]) -> str:
        ...
        # Header
        lines.extend(("# Schema Removals LDIF", "# WARNING: Ensure no data dependencies exist", "", "version: 1", ""))
        ...
        if oc_removals:
            lines.extend(("# Remove Object Classes", "dn: cn=schema", "changetype: modify", "delete: objectClasses"))

            lines.extend(f"objectClasses: {diff.source_definition}" for diff in oc_removals if diff.source_definition)

            lines.extend(("-", ""))

        # Remove attribute types
        if attr_removals:
            lines.extend(("# Remove Attribute Types", "dn: cn=schema", "changetype: modify", "delete: attributeTypes"))

            lines.extend(f"attributeTypes: {diff.source_definition}" for diff in attr_removals if diff.source_definition)
