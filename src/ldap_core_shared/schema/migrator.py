...
from ..ldif.writer import LDIFHeaderConfig, LDIFWriter
from .comparator import DifferenceType, SchemaComparisonResult, SchemaDifference
...
class MigrationPlan(BaseModel):
    ...
    migration_steps: list[str] = Field(default_factory=list, description="Ordered migration steps")
    ldif_files: list[str] = Field(default_factory=list, description="Generated LDIF files")
    warnings: list[str] = Field(default_factory=list, description="Migration warnings")
    estimated_duration: str = Field(default="", description="Estimated migration duration")
...
class SchemaMigrator:
    ...
    def generate_ldif_for_additions(
        ...
        try:
            ...
            ldif_content = self._generate_addition_ldif_content(attr_additions, oc_additions)
            ...
            return LDAPOperationResult[int](
                success=False,
                error_message=f"LDIF generation failed: {str(e)}",
                operation="generate_ldif_for_additions"
            )

    def _create_migration_plan(self, comparison_result: SchemaComparisonResult) -> MigrationPlan:
        ...
        additions = [d for d in comparison_result.differences if d.difference_type == DifferenceType.ADDED]
        removals = [d for d in comparison_result.differences if d.difference_type == DifferenceType.REMOVED]
        modifications = [d for d in comparison_result.differences if d.difference_type == DifferenceType.MODIFIED]
        conflicts = [d for d in comparison_result.differences if d.difference_type == DifferenceType.CONFLICT]
        ...
        if modifications:
            plan.migration_steps.append(f"2. Modify {len(modifications)} existing schema elements")
            plan.warnings.append("Schema modifications may require careful planning")
        
        if removals:
            plan.migration_steps.append(f"3. Remove {len(removals)} obsolete schema elements")
    ...
    def _generate_ldif_files(self, comparison_result: SchemaComparisonResult, output_dir: Path) -> list[str]:
    ...
    def _generate_modification_ldif_content(self, modifications: list[SchemaDifference]) -> str:
