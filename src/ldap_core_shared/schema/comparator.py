...
class SchemaDifference(BaseModel):
    ...
    source_definition: str | None = Field(default=None, description="Definition in source schema")
    target_definition: str | None = Field(default=None, description="Definition in target schema")
...
class SchemaComparisonResult(BaseModel):
    ...
    differences: list[SchemaDifference] = Field(default_factory=list, description="List of differences")
...
class SchemaComparator:
    ...
    def compare_schemas(
        ...
        try:
            ...
            result = SchemaComparisonResult(
                source_elements=len(source_schema.attribute_types) + len(source_schema.object_classes),
                target_elements=len(target_schema.attribute_types) + len(target_schema.object_classes),
    ...
    def _compare_attribute_types(
        ...
        for oid in source_schema.attribute_types.keys():
            if oid in target_schema.attribute_types:
                ...
                if not self._attributes_equal(source_attr, target_attr):
                    differences.append(SchemaDifference(
                        ...
                        difference_type=DifferenceType.MODIFIED,
                        source_definition=self._format_attribute_summary(source_attr),
                        target_definition=self._format_attribute_summary(target_attr),
    ...
    def _compare_object_classes(
        ...
        for oid in source_schema.object_classes.keys():
            if oid in target_schema.object_classes:
                ...
                if not self._object_classes_equal(source_oc, target_oc):
                    differences.append(SchemaDifference(
                        ...
                        difference_type=DifferenceType.MODIFIED,
                        source_definition=self._format_object_class_summary(source_oc),
                        target_definition=self._format_object_class_summary(target_oc),
    ...
    def _find_attribute_by_name(self, attr_type: AttributeType, schema: ParsedSchema) -> AttributeType | None:
    ...
    def _find_object_class_by_name(self, obj_class: ObjectClass, schema: ParsedSchema) -> ObjectClass | None:
    ...
    def _attributes_equal(self, attr1: AttributeType, attr2: AttributeType) -> bool:
        ...
        return (
            attr1.names == attr2.names and
            attr1.description == attr2.description and
            attr1.syntax == attr2.syntax and
            attr1.equality_rule == attr2.equality_rule and
            attr1.single_value == attr2.single_value and
            attr1.usage == attr2.usage
        )

    def _object_classes_equal(self, oc1: ObjectClass, oc2: ObjectClass) -> bool:
        """Check if two object classes are equal."""
        return (
            oc1.names == oc2.names and
            oc1.description == oc2.description and
            oc1.class_type == oc2.class_type and
            set(oc1.superior_classes) == set(oc2.superior_classes) and
            set(oc1.must_attributes) == set(oc2.must_attributes) and
            set(oc1.may_attributes) == set(oc2.may_attributes)
        )
