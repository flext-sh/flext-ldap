...
from ..domain.results import LDAPValidationResult
from .parser import AttributeType, ObjectClass, ParsedSchema
...
class SchemaValidationConfig(BaseModel):
    ...
    check_rfc_compliance: bool = Field(default=True, description="Check RFC 2252 compliance")
    check_dependencies: bool = Field(default=True, description="Check schema dependencies")
    check_name_conflicts: bool = Field(default=True, description="Check for name conflicts")
    check_oid_uniqueness: bool = Field(default=True, description="Check OID uniqueness")
    allow_obsolete_elements: bool = Field(default=False, description="Allow obsolete schema elements")
...
class SchemaValidator:
    ...
    def validate_schema(self, schema: ParsedSchema) -> LDAPValidationResult:
        ...
        for oid, attr_type in schema.attribute_types.items():
            result = self.validate_attribute_type(attr_type, schema)
            errors.extend(result.errors)
            warnings.extend(result.warnings)
        
        # Validate object classes
        for oid, obj_class in schema.object_classes.items():
    ...
    def validate_attribute_type(self, attr_type: AttributeType, schema: ParsedSchema) -> LDAPValidationResult:
    ...
    def validate_object_class(self, obj_class: ObjectClass, schema: ParsedSchema) -> LDAPValidationResult:
    ...
    def _check_attribute_dependencies(self, attr_type: AttributeType, schema: ParsedSchema) -> list[str]:
        ...
        if attr_type.superior:
            if not self._find_attribute_by_name(attr_type.superior, schema):
                errors.append(f"Superior attribute type not found: {attr_type.superior}")
        ...
        if attr_type.equality_rule and schema.matching_rules:
            if not self._find_matching_rule_by_name(attr_type.equality_rule, schema):
                errors.append(f"Equality matching rule not found: {attr_type.equality_rule}")
        
        return errors

    def _check_object_class_dependencies(self, obj_class: ObjectClass, schema: ParsedSchema) -> list[str]:
    ...
    def _check_oid_uniqueness(self, schema: ParsedSchema) -> list[str]:
        ...
        for oid in schema.attribute_types.keys():
            if oid in all_oids:
                errors.append(f"Duplicate OID found: {oid}")
            all_oids.add(oid)
        
        # Check object classes
        for oid in schema.object_classes.keys():
            if oid in all_oids:
                errors.append(f"Duplicate OID found: {oid}")
            all_oids.add(oid)
        
        # Check syntax definitions
        for oid in schema.syntax_definitions.keys():
            if oid in all_oids:
                errors.append(f"Duplicate OID found: {oid}")
            all_oids.add(oid)
        
        # Check matching rules
        for oid in schema.matching_rules.keys():
            if oid in all_oids:
                errors.append(f"Duplicate OID found: {oid}")
            all_oids.add(oid)
    ...
    def _check_name_conflicts(self, schema: ParsedSchema) -> list[str]:
        ...
        for attr_type in schema.attribute_types.values():
            for name in attr_type.names:
                ...
                if name_lower in all_names:
                    errors.append(f"Name conflict: '{name}' used in both {all_names[name_lower]} and attribute type {attr_type.oid}")
                else:
        ...
        for obj_class in schema.object_classes.values():
            for name in obj_class.names:
                name_lower = name.lower()
                if name_lower in all_names:
                    errors.append(f"Name conflict: '{name}' used in both {all_names[name_lower]} and object class {obj_class.oid}")
                else:
    ...
    def _find_attribute_by_name(self, name: str, schema: ParsedSchema) -> AttributeType | None:
    ...
    def _find_object_class_by_name(self, name: str, schema: ParsedSchema) -> ObjectClass | None:
