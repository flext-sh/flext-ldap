...
from .parser import ParsedSchema
...
class SchemaAnalysisResult(BaseModel):
    ...
    statistics: dict[str, Any] = Field(default_factory=dict, description="Detailed statistics")
...
class SchemaAnalyzer:
    ...
    def analyze_schema(self, schema: ParsedSchema) -> LDAPOperationResult[SchemaAnalysisResult]:
    ...
    def _generate_recommendations(self, schema: ParsedSchema) -> list[str]:
        ...
        if obsolete_attrs or obsolete_ocs:
            recommendations.append(f"Remove {len(obsolete_attrs + obsolete_ocs)} obsolete schema elements")
        ...
        if attrs_without_desc:
            recommendations.append(f"Add descriptions to {len(attrs_without_desc)} attribute types")
        
        if ocs_without_desc:
            recommendations.append(f"Add descriptions to {len(ocs_without_desc)} object classes")
        ...
        if attrs_without_syntax:
            recommendations.append(f"Define syntax for {len(attrs_without_syntax)} attribute types")
        
        # Check for naming consistency
        naming_issues = self._check_naming_consistency(schema)
        if naming_issues:
            recommendations.append("Improve naming consistency for better maintainability")
        ...
        defined_attributes = {name for attr in schema.attribute_types.values() for name in attr.names}
        unused_attributes = defined_attributes - used_attributes
        
        if unused_attributes:
            recommendations.append(f"Review {len(unused_attributes)} unused attribute definitions")
    ...
    def _generate_statistics(self, schema: ParsedSchema) -> dict[str, Any]:
        ...
        single_valued_attrs = sum(1 for attr in schema.attribute_types.values() if attr.single_value)
        ...
        statistics.update({
            ...
            "obsolete_attributes": sum(1 for attr in schema.attribute_types.values() if attr.obsolete),
        })
        
        # Object class statistics
        structural_ocs = sum(1 for oc in schema.object_classes.values() if oc.class_type == "STRUCTURAL")
        auxiliary_ocs = sum(1 for oc in schema.object_classes.values() if oc.class_type == "AUXILIARY")
        abstract_ocs = sum(1 for oc in schema.object_classes.values() if oc.class_type == "ABSTRACT")
        
        statistics.update({
            "structural_object_classes": structural_ocs,
            "auxiliary_object_classes": auxiliary_ocs,
            "abstract_object_classes": abstract_ocs,
            "obsolete_object_classes": sum(1 for oc in schema.object_classes.values() if oc.obsolete),
        })
    ...
    def _check_naming_consistency(self, schema: ParsedSchema) -> list[str]:
        ...
        if camel_case_names and kebab_case_names:
            issues.append("Mixed naming conventions detected (camelCase and kebab-case)")
