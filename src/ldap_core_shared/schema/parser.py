...
class AttributeType(BaseModel):
    ...
    no_user_modification: bool = Field(default=False, description="No user modification")
...
class ObjectClass(BaseModel):
    ...
    superior_classes: list[str] = Field(default_factory=list, description="Superior object classes")
    class_type: str = Field(default="STRUCTURAL", description="Object class type")
    must_attributes: list[str] = Field(default_factory=list, description="Required attributes")
    may_attributes: list[str] = Field(default_factory=list, description="Optional attributes")
...
class ParsedSchema(BaseModel):
    ...
    attribute_types: dict[str, AttributeType] = Field(default_factory=dict, description="Attribute types by OID")
    object_classes: dict[str, ObjectClass] = Field(default_factory=dict, description="Object classes by OID")
    syntax_definitions: dict[str, SyntaxDefinition] = Field(default_factory=dict, description="Syntax definitions by OID")
    matching_rules: dict[str, MatchingRule] = Field(default_factory=dict, description="Matching rules by OID")
...
class SchemaParser:
    ...
    def __init__(self) -> None:
        ...
        self._name_pattern = re.compile(r"NAME\s+(?:'([^']+)'|\(([^)]+)\))", re.IGNORECASE)
        self._desc_pattern = re.compile(r"DESC\s+'([^']*)'", re.IGNORECASE)
        self._syntax_pattern = re.compile(r"SYNTAX\s+([0-9.]+)(?:\{(\d+)\})?", re.IGNORECASE)
        self._equality_pattern = re.compile(r"EQUALITY\s+([a-zA-Z0-9-]+)", re.IGNORECASE)
        self._ordering_pattern = re.compile(r"ORDERING\s+([a-zA-Z0-9-]+)", re.IGNORECASE)
    ...
    def parse_attribute_type(self, definition: str) -> LDAPOperationResult[AttributeType]:
    ...
    def parse_syntax_definition(self, definition: str) -> LDAPOperationResult[SyntaxDefinition]:
    ...
    def _extract_superior_classes(self, definition: str) -> list[str]:
        ...
        match = re.search(r"SUP\s+(?:([a-zA-Z0-9-]+)|\(([^)]+)\))", definition, re.IGNORECASE)
        if not match:
            return []
    ...
    def _extract_class_type(self, definition: str) -> str:
        ...
        if "ABSTRACT" in definition.upper():
            return "ABSTRACT"
        elif "AUXILIARY" in definition.upper():
            return "AUXILIARY"
        else:
            return "STRUCTURAL"
