...
class SearchFilter(BaseModel):
    ...
    def and_(self, other: SearchFilter) -> SearchFilter:
        ...
        return SearchFilter(filter_string=f"(&{self.filter_string}{other.filter_string})")
...
class LDAPSearchEngine:
    """Enterprise LDAP search engine with advanced capabilities."""
                ...
                if success:
                    ...
                    if config.page_size and hasattr(connection, 'response'):
                        ...
                        for control in connection.response.get('controls', []):
                            if control.get('controlType') == '1.2.840.113556.1.4.319':  # Paged results control
                    ...
                    return LDAPSearchResult(
                        ...
                        errors=[f"Search failed: {connection.result.get('description', 'Unknown error')}"],
                    )
