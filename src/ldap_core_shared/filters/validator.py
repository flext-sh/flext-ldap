...
from .parser import FilterParser, FilterSyntaxError, ParsedFilter, FilterType
...
class ValidationIssue(BaseModel):
    ...
    code: str = Field(
        description="Unique issue code for programmatic handling"
    )
...
class FilterValidationResult(BaseModel):
    ...
    is_valid: bool = Field(
        description="Whether filter passed all validation checks"
    )
    
    filter_string: str = Field(
        description="Original filter string that was validated"
    )
    ...
    complexity_score: int = Field(
        default=0,
        description="Filter complexity score"
    )
...
class SecurityRule(ValidationRule):
    ...
    def _is_overly_broad(self, parsed_filter: ParsedFilter) -> bool:
        ...
        if (parsed_filter.filter_type == FilterType.SUBSTRING and 
            parsed_filter.value and parsed_filter.value.count("*") > 2):
            return True
        ...
        def check_schema(filter_node: ParsedFilter) -> None:
            if filter_node.attribute:
                ...
                if filter_node.attribute not in self.schema:
                    ...
                    if (filter_node.operator in [">=", "<="] and 
                        "numeric" not in syntax.lower() and 
                        "time" not in syntax.lower()):
                        issues.append(ValidationIssue(
                            severity=ValidationSeverity.WARNING,
...
class FilterValidator:
    ...
    Example:
        ...
        >>> result = validator.validate("(&(cn=john)(mail=*REDACTED_LDAP_BIND_PASSWORD*))")
        ...
        for rule in self._rules:
            try:
                ...
                issues.append(ValidationIssue(
                    ...
                    suggestion="Contact REDACTED_LDAP_BIND_PASSWORDistrator about validation rule configuration"
                ))
    ...
    def _initialize_rules(self, custom_rules: list[ValidationRule]) -> list[ValidationRule]:
        ...
        if self.level in {ValidationLevel.STANDARD, ValidationLevel.STRICT, ValidationLevel.ENTERPRISE}:
    ...
    def _calculate_performance_rating(
        ...
        if complexity <= 5 and not performance_issues:
            return "excellent"
        elif complexity <= 15 and len(performance_issues) <= 1:
            return "good"
        elif complexity <= 30 and len(performance_issues) <= 3:
            return "fair"
        else:
            return "poor"
