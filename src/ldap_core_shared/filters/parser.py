...
Usage Example:
    ...
    >>> complex_filter = "(&(objectClass=person)(|(cn=*admin*)(mail=*@example.com)))"
...
class ParsedFilter(BaseModel):
    ...
    filter_type: FilterType = Field(
        description="Type of LDAP filter"
    )
    ...
    is_negated: bool = Field(
        default=False,
        description="Whether filter is negated"
    )
    
    raw_filter: str = Field(
        description="Original filter string"
    )
    ...
    def validate_children(cls, v: list, values: dict) -> list:
        ...
        if filter_type == FilterType.AND and len(v) < 2:
            raise ValueError("AND filter must have at least 2 children")
        
        if filter_type == FilterType.OR and len(v) < 2:
            raise ValueError("OR filter must have at least 2 children")
        
        if filter_type == FilterType.NOT and len(v) != 1:
            raise ValueError("NOT filter must have exactly 1 child")
...
class FilterParser:
    ...
    def parse(self, filter_string: str) -> ParsedFilter:
        ...
        if not filter_string or not filter_string.strip():
            raise FilterSyntaxError("Empty filter string")
    ...
    def _parse_and_filter(self) -> ParsedFilter:
        ...
        if len(children) < 2:
            raise FilterSyntaxError(
                "AND filter requires at least 2 child filters",
                self._position,
                self._filter_string
            )
    ...
    def _parse_or_filter(self) -> ParsedFilter:
        ...
        if len(children) < 2:
            raise FilterSyntaxError(
                "OR filter requires at least 2 child filters",
                self._position,
                self._filter_string
            )
    ...
    def _parse_not_filter(self) -> ParsedFilter:
        ...
        if not self._has_more() or self._current_char() == ")":
            raise FilterSyntaxError(
                "NOT filter requires a child filter",
                self._position,
                self._filter_string
            )
    ...
    def _parse_simple_filter(self) -> ParsedFilter:
        ...
        if not self._has_more():
            raise FilterSyntaxError(
                "Incomplete filter after attribute",
                self._position,
                self._filter_string
            )
...
class FilterAnalyzer:
    ...
    Example:
        ...
        >>> analysis = analyzer.analyze("(&(objectClass=person)(cn=*admin*))")
    ...
    def _analyze_performance(self, parsed: ParsedFilter) -> list[str]:
        ...
        if self._has_substring_filters(parsed):
            hints.append("Substring filters may impact performance - consider using indexed attributes")
        
        # Check for complex nested filters
        if parsed.get_complexity_score() > 10:
            hints.append("Complex nested filter - consider simplification for better performance")
    ...
    def _analyze_security(self, parsed: ParsedFilter) -> list[str]:
        ...
        if self._is_overly_broad(parsed):
            warnings.append("Filter may return excessive results - consider adding more constraints")
        
        # Check for potential injection patterns
        if self._has_injection_patterns(parsed):
            warnings.append("Filter contains patterns that may indicate injection attempts")
    ...
    def _suggest_optimizations(self, parsed: ParsedFilter) -> list[str]:
        ...
        if parsed.filter_type == FilterType.AND:
            suggestions.append("Consider placing equality filters before substring filters")
        
        # Suggest using presence filters for existence checks
        if self._can_use_presence_filter(parsed):
            suggestions.append("Consider using presence filter (*) for attribute existence checks")
