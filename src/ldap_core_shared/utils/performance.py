...
class LDAPMetrics(BaseModel):
    ...
    def meets_sla(self) -> bool:
        ...
        return (
            ...
            and self.operations_per_second >= TARGET_OPERATIONS_PER_SECOND * 0.8  # 80% of target
        )
...
class ConnectionPoolMetrics(BaseModel):
    ...
    def efficiency_grade(self) -> str:
        ...
        if (
            self.reuse_rate >= TARGET_CONNECTION_REUSE_RATE * PERCENTAGE_CALCULATION_BASE
...
class PerformanceAnalyzer:
    ...
    def detect_performance_degradation(self, threshold: float = 0.2) -> dict[str, Any]:
        ...
        # Calculate averages
        recent_avg_ops_per_sec = sum(m.operations_per_second for m in recent_metrics) / len(recent_metrics)
        baseline_avg_ops_per_sec = sum(m.operations_per_second for m in baseline_metrics) / len(baseline_metrics)
        
        recent_avg_duration = sum(m.average_duration for m in recent_metrics) / len(recent_metrics)
        baseline_avg_duration = sum(m.average_duration for m in baseline_metrics) / len(baseline_metrics)
        ...
        if baseline_avg_ops_per_sec > 0:
            ops_change = (baseline_avg_ops_per_sec - recent_avg_ops_per_sec) / baseline_avg_ops_per_sec
    ...
    def generate_performance_report(self) -> dict[str, Any]:
        ...
        avg_ops_per_second = sum(m.operations_per_second for m in self._metrics_history) / len(self._metrics_history)
        avg_duration = sum(m.average_duration for m in self._metrics_history) / len(self._metrics_history)
        
        overall_success_rate = (total_successes / total_operations * PERCENTAGE_CALCULATION_BASE) if total_operations > 0 else 0
        
        # Performance grade
        performance_grade = self._calculate_performance_grade(avg_ops_per_second, overall_success_rate, avg_duration)
