...
from flext_ldif.quirks import FlextLdifQuirksManager, FlextLdifEntryQuirks
...
class FlextLdapEntryAdapter(FlextService[None]):
    ...
    def __init__(self, server_type: str | None = None) -> None:
        """Initialize entry adapter with FlextLdif integration and quirks support.
    ...
    def ldap3_to_ldif_entry(
        ...
        try:
            ...
            # Create FlextLdif Entry using the model
            try:
                ...
                for attr_name, attr_value_list in attributes.items():
                    attr_values_dict[attr_name] = FlextLdifModels.AttributeValues(
                ...
                return FlextResult[FlextLdifModels.Entry].fail(
                    f"Failed to create FlextLdif entry: {model_error}"
                )
            ...
            return FlextResult[FlextLdifModels.Entry].fail(
                f"ldap3 to FlextLdif conversion failed: {e}"
            )
    ...
    def ldif_entry_to_ldap3_attributes(
        ...
        try:
            if not ldif_entry:
                return FlextResult[dict[str, list[Any]]].fail(
            ...
            return FlextResult[dict[str, list[Any]]].ok(attributes)
            ...
            self._logger.error(
                ...
                extra={
                    "error": str(e),
                    "dn": str(ldif_entry.dn) if ldif_entry else None,
                },
            )
            return FlextResult[dict[str, list[Any]]].fail(
                f"FlextLdif to ldap3 conversion failed: {e}"
            )
    ...
    def normalize_attributes_for_add(
        ...
        try:
            ...
            return FlextResult[dict[str, list[Any]]].ok(normalized)

        except Exception as e:
            return FlextResult[dict[str, list[Any]]].fail(
                f"Attribute normalization failed: {e}"
            )

    def create_modify_changes(
        self, modifications: dict[str, Any]
    ) -> FlextResult[dict[str, list[tuple[str, list[Any]]]]]:
        ...
        try:
            ...
            changes: dict[str, list[tuple[str, list[Any]]]] = {}
            ...
            return FlextResult[dict[str, list[tuple[str, list[Any]]]]].ok(changes)

        except Exception as e:
            return FlextResult[dict[str, list[tuple[str, list[Any]]]]].fail(
                f"Failed to create modify changes: {e}"
            )
    ...
    def convert_ldif_file_to_entries(
        ...
        try:
            ...
            return FlextResult[list[FlextLdifModels.Entry]].fail(
                f"LDIF file conversion failed: {e}"
            )
    ...
    def write_entries_to_ldif_file(
        ...
        try:
            ...
            return FlextResult[str].fail(f"LDIF file writing failed: {e}")
        ...
        try:
            ...
            self._logger.debug(
                ...
                extra={
                    "dn": str(entry.dn),
                    "target_server": target_server_type,
                    "attributes_count": entry.attributes.attribute_count,
                },
            )
    ...
    def convert_entry_format(
        ...
        try:
            ...
            normalize_result = self.normalize_entry_for_server(
                entry, target_server_type
            )
            ...
            return FlextResult[FlextLdifModels.Entry].fail(
                f"Entry format conversion failed: {e}"
            )
    ...
    def get_server_specific_attributes(
        ...
        try:
            ...
            return FlextResult[dict[str, Any]].ok(server_attrs)
