...
class FlextLdapClient:
    ...
    async def connect(
        ...
        try:
            ...
            return await self._perform_connection_sequence(
    ...
    async def _perform_connection_sequence(
        ...
        logger.info(
            FlextLdapOperationMessages.CONNECTION_ESTABLISHED.format(
                session_id=server_uri
            ),
            extra={
    ...
    async def disconnect(self, *args: object, **kwargs: object) -> FlextResult[None]:
        ...
        try:
            ...
            if conn is not None and hasattr(conn, "unbind"):
                try:
                    # Provide local callable for ldap3 unbind without top-level import
                    unbind = cast("object", conn.unbind)
                    if callable(unbind):
                        unbind()
    ...
    async def _handle_keyword_search(
        ...
        return await self._search_impl(
            ...
            int(search_params["time_limit"])
            if isinstance(search_params["time_limit"], int)
            else 30,
        )
    ...
    async def _handle_convenience_positional_search(
        ...
        return await self.search_convenience(
            ...
            time_limit=int(convenience_params["time_limit"])
            if isinstance(convenience_params["time_limit"], int)
            else 30,
        )
    ...
    async def _handle_modern_positional_search(
        ...
        return await self._search_impl(
            ...
            int(modern_params["time_limit"])
            if isinstance(modern_params["time_limit"], int)
            else 30,
        )

    def _extract_search_params_from_kwargs(
        self, kwargs: dict[str, object]
    ) -> dict[str, object]:
        ...
        return {
            ...
            "size_limit": self._safe_int_conversion(
                kwargs.get("size_limit", 1000), 1000
            ),
            "time_limit": self._safe_int_conversion(kwargs.get("time_limit", 30), 30),
        }

    def _extract_convenience_search_params(
        self, kwargs: dict[str, object]
    ) -> dict[str, object]:
    ...
    def _extract_modern_search_params(
        self, kwargs: dict[str, object]
    ) -> dict[str, object]:
...
class FlextLdapPagedSearchStrategy(FlextLdapSearchStrategy):
    ...
    async def execute_search(
        ...
        **kwargs: object,
        ...
        return await standard_strategy.execute_search(
            client, base_dn, search_filter, **kwargs
        )
...
class FlextLdapObservableClient(FlextLdapClient):
    ...
    async def connect(
        ...
        if result.is_success:
            ...
            await self._notify_connection_failed(
                server_uri, result.error or "Connection failed"
            )
...
class FlextLdapStrategyContext:
    ...
    async def execute_search(
        ...
        **kwargs: object,
        ...
        return await self._strategy.execute_search(
            client, base_dn, search_filter, **kwargs
        )
