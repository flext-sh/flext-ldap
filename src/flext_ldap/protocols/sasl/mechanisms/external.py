...
Usage Example:
    >>> from flext_ldap.protocols.sasl.mechanisms.external import ExternalMechanism
    >>> from flext_ldapasl.callback import SASLCallbackHandler
    >>>
    >>> # With authorization identity
    >>> callback = SASLCallbackHandler(authorization_id="cn=admin,dc=example,dc=com")
...
if TYPE_CHECKING:
    from flext_ldapasl.context import SASLContext
...
class ExternalMechanism(SASLMechanism):
    ...
    Example:
        ...
        >>> callback.authorization_id = "cn=admin,dc=example,dc=com"
        >>> response = mechanism.get_initial_response()  # b'cn=admin,dc=example,dc=com'
    """
