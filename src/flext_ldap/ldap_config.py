...
class FlextLdapConnectionConfig(FlextLDAPConfig):
    ...
    @classmethod
    def validate_port_number(cls, v: int, values: dict[str, object] | None = None) -> int:
    ...
    def with_server(self, host: str, port: int | None = None) -> FlextLdapConnectionConfig:
...
class FlextLdapSettings(FlextBaseConfigModel):
    ...
    enable_test_mode: bool = Field(
        default=False,
        description="Enable test mode with mocked operations",
    )

    def validate_configuration(self) -> FlextResult[None]:
        """Validate complete settings configuration."""
        if self.default_connection:
            # Validate connection can be established
    ...
    def get_effective_connection(self, override: FlextLdapConnectionConfig | None = None) -> FlextLdapConnectionConfig:
...
class FlextLdapAuthConfig(FlextBaseConfigModel):
    ...
    bind_dn: str = Field(description="Bind Distinguished Name")
    bind_password: SecretStr | None = Field(default=None, description="Bind password")
