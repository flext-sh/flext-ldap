...
class FlextLdapUpsertService(FlextService[dict[str, object]]):
    ...
    def upsert_entry(
        ...
        search_result = ldap_client.client.search(
            base_dn=dn,
        ...
        # Extract Entry from search response (search() returns list of (dn, Entry) tuples | Entry | None)
        # Handle three cases: None, single Entry, list of (dn, Entry) tuples
        if isinstance(search_response, list):
            ...
            # search_response[0] is a tuple (dn, Entry), extract the Entry
            ...
            if (
                ...
                # Format: (dn_string, Entry) - extract Entry at index 1
                existing_entry: FlextLdifModels.Entry = search_result_item[entry_index]
            else:
                # Direct Entry object
                existing_entry = search_result_item
        else:
        ...
        if to_add:
            # Build changes dict with MODIFY_ADD operation tuples
            add_changes_dict = {
            ...
            add_result = ldap_client.client.modify_entry(
                dn=dn,
                changes=add_changes,
            )
        ...
        if to_replace:
            # Build changes dict with MODIFY_REPLACE operation tuples
            ...
            replace_result = ldap_client.client.modify_entry(
                dn=dn,
