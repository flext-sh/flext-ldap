...
class FlextLdapOperationsBase:
    ...
    def _log_operation_success(
        ...
        **extra_fields: object,
        ...
        logger.info(f"LDAP {operation} completed", extra={
...
class FlextLdapConnectionOperations(FlextLdapOperationsBase):
    ...
    async def create_connection(
        ...
        try:
            ...
            self._log_operation_success("connection created", connection_id,
                server_uri=server_uri, authenticated=bind_dn is not None)
    ...
    async def close_connection(self, connection_id: str) -> FlextResult[None]:
        ...
        try:
            ...
            # Use REFACTORED logging - NO DUPLICATION
            self._log_operation_success("connection closed", connection_id,
                server_uri=connection_info.get("server_uri"),
                duration_seconds=self._calculate_duration(connection_info.get("created_at")))

            return FlextResult.ok(None)

        except Exception as e:
            # Use REFACTORED exception handling - NO DUPLICATION
            error_msg = self._handle_exception_with_context("close connection", e, connection_id)
            return FlextResult.fail(error_msg)
...
class FlextLdapSearchOperations(FlextLdapOperationsBase):
    ...
    async def search_entries(
        ...
        try:
            ...
            if not filter_validation.is_success:
                return FlextResult.fail(filter_validation.error or "Filter validation failed")
            ...
            # Use REFACTORED exception handling - NO DUPLICATION
            error_msg = self._handle_exception_with_context("search operation", e, connection_id)
            return FlextResult.fail(error_msg)
    ...
    async def search_users(
        ...
        try:
            ...
            # Use REFACTORED logging - NO DUPLICATION
            self._log_operation_success("user search", connection_id,
                base_dn=base_dn, criteria=filter_criteria, result_count=len(users))

            return FlextResult.ok(users)

        except Exception as e:
            # Use REFACTORED exception handling - NO DUPLICATION
            error_msg = self._handle_exception_with_context("user search", e, connection_id)
            return FlextResult.fail(error_msg)
    ...
    async def search_groups(
        ...
        try:
            ...
            # Use REFACTORED logging - NO DUPLICATION
            self._log_operation_success("group search", connection_id,
                base_dn=base_dn, criteria=filter_criteria, result_count=len(groups))

            return FlextResult.ok(groups)

        except Exception as e:
            # Use REFACTORED exception handling - NO DUPLICATION
            error_msg = self._handle_exception_with_context("group search", e, connection_id)
            return FlextResult.fail(error_msg)
    ...
    def _convert_entries_to_users(self, entries: list[FlextLdapEntry]) -> list[FlextLdapUser]:
    ...
    def _convert_entries_to_groups(self, entries: list[FlextLdapEntry]) -> list[FlextLdapGroup]:
...
class FlextLdapEntryOperations(FlextLdapOperationsBase):
    ...
    async def create_entry(
        ...
        try:
            ...
            # Use REFACTORED logging - NO DUPLICATION
            self._log_operation_success("entry created", connection_id,
                entry_dn=dn, object_classes=object_classes, attribute_count=len(attributes))

            return FlextResult.ok(entry)

        except Exception as e:
            # Use REFACTORED exception handling - NO DUPLICATION
            error_msg = self._handle_exception_with_context("create entry", e, connection_id)
            return FlextResult.fail(error_msg)
    ...
    async def modify_entry(
        ...
        try:
            ...
            # Use REFACTORED logging - NO DUPLICATION
            self._log_operation_success("entry modified", connection_id,
                entry_dn=dn, modification_count=len(modifications))

            return FlextResult.ok(None)

        except Exception as e:
            # Use REFACTORED exception handling - NO DUPLICATION
            error_msg = self._handle_exception_with_context("modify entry", e, connection_id)
            return FlextResult.fail(error_msg)
    ...
    async def delete_entry(
        ...
        try:
            ...
            # Use REFACTORED exception handling - NO DUPLICATION
            error_msg = self._handle_exception_with_context("delete entry", e, connection_id)
            return FlextResult.fail(error_msg)
...
class FlextLdapUserOperations(FlextLdapOperationsBase):
    ...
    async def create_user(
        ...
        try:
            ...
            if not entry_result.is_success:
                return FlextResult.fail(f"Failed to create user entry: {entry_result.error}")
            ...
            # Use REFACTORED logging - NO DUPLICATION
            self._log_operation_success("user created", connection_id,
                user_dn=user_request.dn, uid=user_request.uid)

            return FlextResult.ok(user)

        except Exception as e:
            # Use REFACTORED exception handling - NO DUPLICATION
            error_msg = self._handle_exception_with_context("create user", e, connection_id)
            return FlextResult.fail(error_msg)
    ...
    async def update_user_password(
        ...
        if not new_password or len(new_password) < self.MIN_PASSWORD_LENGTH:
            return FlextResult.fail(f"Password must be at least {self.MIN_PASSWORD_LENGTH} characters")

        modifications = {"userPassword": [new_password]}
    ...
    async def activate_user(self, connection_id: str, user_dn: str) -> FlextResult[None]:
        """Activate user account - REFACTORED."""
        modifications = {"accountStatus": ["active"]}
        return await self._entry_ops.modify_entry(connection_id, user_dn, modifications)

    async def deactivate_user(self, connection_id: str, user_dn: str) -> FlextResult[None]:
        """Deactivate user account - REFACTORED."""
        modifications = {"accountStatus": ["inactive"]}
        return await self._entry_ops.modify_entry(connection_id, user_dn, modifications)

    def _build_user_attributes(self, user_request: FlextLdapCreateUserRequest) -> dict[str, list[str]]:
...
class FlextLdapGroupOperations(FlextLdapOperationsBase):
    ...
    async def create_group(
        ...
        try:
            ...
            if not entry_result.is_success:
                return FlextResult.fail(f"Failed to create group entry: {entry_result.error}")
            ...
            # Use REFACTORED logging - NO DUPLICATION
            self._log_operation_success("group created", connection_id,
                group_dn=dn, cn=cn, member_count=len(members))

            return FlextResult.ok(group)

        except Exception as e:
            # Use REFACTORED exception handling - NO DUPLICATION
            error_msg = self._handle_exception_with_context("create group", e, connection_id)
            return FlextResult.fail(error_msg)
    ...
    async def add_group_member(
        ...
        try:
            ...
            if not member_validation.is_success:
                return FlextResult.fail(member_validation.error or "Member validation failed")

            # Use REFACTORED member management - NO DUPLICATION
            return await self._modify_group_membership(
                connection_id, group_dn, member_dn, action="add",
            )

        except Exception as e:
            # Use REFACTORED exception handling - NO DUPLICATION
            error_msg = self._handle_exception_with_context("add group member", e, connection_id)
            return FlextResult.fail(error_msg)
    ...
    async def remove_group_member(
        ...
        try:
            # Use REFACTORED member management - NO DUPLICATION
            return await self._modify_group_membership(
                connection_id, group_dn, member_dn, action="remove",
            )

        except Exception as e:
            # Use REFACTORED exception handling - NO DUPLICATION
            error_msg = self._handle_exception_with_context("remove group member", e, connection_id)
            return FlextResult.fail(error_msg)
    ...
    async def get_group_members(
        ...
        try:
            ...
            # Use REFACTORED exception handling - NO DUPLICATION
            error_msg = self._handle_exception_with_context("get group members", e, connection_id)
            return FlextResult.fail(error_msg)
    ...
    async def update_group_description(
        ...
        return await self._entry_ops.modify_entry(connection_id, group_dn, modifications)
    ...
    async def _modify_group_membership(
        ...
        if modify_result.is_success:
            ...
            self._log_operation_success(f"member {action_verb} group", connection_id,
                group_dn=group_dn, member_dn=member_dn)
