...
LDAP directories, supporting both self-service and REDACTED_LDAP_BIND_PASSWORDistrative password changes.
...
Usage Example:
    >>> from flext_ldap.extensions.modify_password import ModifyPasswordExtension
    >>>
    ...
    >>> REDACTED_LDAP_BIND_PASSWORD_reset = ModifyPasswordExtension(
    ...     user_identity="uid=jdoe,ou=people,dc=example,dc=com",
    ...     new_password="temp_password",
    ... )
    >>> result = connection.extended_operation(REDACTED_LDAP_BIND_PASSWORD_reset)
    >>>
...
from flext_ldapants import BER_CONTEXT_TAG_0, BER_SEQUENCE_TAG
...
class ModifyPasswordExtension(LDAPExtension):
    ...
    LDAP directories. It supports self-service password changes, REDACTED_LDAP_BIND_PASSWORDistrative
    ...
    def is_REDACTED_LDAP_BIND_PASSWORD_operation(self) -> bool:
        """Check if this is an REDACTED_LDAP_BIND_PASSWORDistrative password operation."""
    ...
    def __str__(self) -> str:
        ...
        if self.is_password_generation():
            return f"ModifyPassword(REDACTED_LDAP_BIND_PASSWORD, generate for {self.user_identity})"
        return f"ModifyPassword(REDACTED_LDAP_BIND_PASSWORD, reset for {self.user_identity})"
