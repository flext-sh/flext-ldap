...
class BaseRulesManager(ABC):
    ...
    def _load_rules(self) -> None:
        ...
        try:
            ...
            logger.info("✅ Loaded rules from %s", self.rules_path)

        except Exception as e:
            logger.error("Failed to load rules from {self.rules_path}: %s", e)
            self._raw_rules = {}
    ...
    def get_rule_section(self, section_name: str) -> dict[str, Any]:
        ...
        result: dict[str, Any] = self.raw_rules.get(section_name, {})
        return result
...
def validate_rules_file(rules_path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        msg = f"Invalid JSON in rules file: {e}"
    ...
    if "categorization_rules" in rules_data:
        ...
        if not isinstance(categorization_rules, dict):
            validation_results["errors"].append(
                "categorization_rules must be a dictionary",
            )
        else:
