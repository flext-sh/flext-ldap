...
class TestOIDOperationsFullCoverage:
    ...
    def test_oid_search_withs_mode(self, shared_ldap_client: FlextLdapClients) -> None:
...
class TestOUDOperationsFullCoverage:
    ...
    def test_oud_admin_operations_search(
        self, shared_ldap_client: FlextLdapClients
    ) -> None:
        """Test OUD admin operations search."""
        result = shared_ldap_client.search(
            base_dn="dc=flext,dc=local",
            filter_str="(&(objectClass=*)(cn=admin*))",
            scope="SUBTREE",
        )
        assert isinstance(result, FlextResult)
