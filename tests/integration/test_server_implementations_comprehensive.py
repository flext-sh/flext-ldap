...
class TestOpenLDAP2SpecificOperations:
    ...
    def test_openldap_modify_entry_attributes(
        ...
        result = shared_ldap_client.modify_entry(
            ...
            changes={"description": ["OpenLDAP admin entry"]},
        )
        assert result.is_success or result.is_failure
...
class TestOpenLDAP1SpecificOperations:
    ...
    def test_openldap1_entry_retrieval(
        ...
        result = shared_ldap_client.get_user(dn="cn=admin,dc=flext,dc=local")
        assert result.is_success or result.is_failure
...
class TestOracleOUDSpecificOperations:
    ...
    def test_oud_member_operations(self, shared_ldap_client: FlextLdapClients) -> None:
        ...
        result = shared_ldap_client.add_member(
            group_dn="cn=admins,ou=groups,dc=flext,dc=local",
...
class TestServerAttributeNormalization:
    ...
    def test_normalize_dn_variations(
        ...
        dn1 = shared_ldap_client.normalize_dn("cn=admin,dc=flext,dc=local")
        dn2 = shared_ldap_client.normalize_dn("CN=admin,DC=flext,DC=local")
...
class TestServerSpecificSearchFilters:
    ...
    def test_search_with_extensible_filter(
        ...
        result = shared_ldap_client.search(
            base_dn="dc=flext,dc=local",
            filter_str="(cn:caseIgnoreMatch:=admin)",
        )
        assert result.is_success or result.is_failure
...
class TestServerBulkOperations:
    ...
    def test_bulk_search_all_entries(
        ...
        if result.is_success:
            ...
            # Should find at least base and admin entries
