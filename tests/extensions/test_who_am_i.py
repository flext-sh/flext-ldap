...
class TestAuthorizationIdentityParser:
    ...
    def test_parse_raw_dn_format(self) -> None:
        ...
        identity_string = "cn=REDACTED_LDAP_BIND_PASSWORD,dc=example,dc=com"
        identity_type, value = AuthorizationIdentityParser.parse(identity_string)

        assert identity_type == IdentityType.DN
        assert value == "cn=REDACTED_LDAP_BIND_PASSWORD,dc=example,dc=com"

    def test_parse_raw_dn_complex(self) -> None:
        """Test parsing complex raw DN."""
        identity_string = "uid=REDACTED_LDAP_BIND_PASSWORD,ou=people,dc=example,dc=org"
        identity_type, value = AuthorizationIdentityParser.parse(identity_string)

        assert identity_type == IdentityType.DN
        assert value == "uid=REDACTED_LDAP_BIND_PASSWORD,ou=people,dc=example,dc=org"
...
class TestWhoAmIResult:
    ...
    def test_result_creation_dn_identity(self) -> None:
        ...
        dn_string = "dn:cn=REDACTED_LDAP_BIND_PASSWORD,dc=example,dc=com"
        result = WhoAmIResult(
            result_code=0,
        ...
        assert result.identity_value == "cn=REDACTED_LDAP_BIND_PASSWORD,dc=example,dc=com"
    ...
    def test_get_display_name_method(self) -> None:
        ...
        dn_result = WhoAmIResult(
            result_code=0,
            authorization_identity="dn:cn=REDACTED_LDAP_BIND_PASSWORD,dc=example,dc=com",
        )
        assert dn_result.get_display_name() == "DN: cn=REDACTED_LDAP_BIND_PASSWORD,dc=example,dc=com"

        # User ID identity
        userid_result = WhoAmIResult(
            result_code=0,
            authorization_identity="u:REDACTED_LDAP_BIND_PASSWORD",
        )
        assert userid_result.get_display_name() == "User: REDACTED_LDAP_BIND_PASSWORD"
    ...
    def test_str_representation_success(self) -> None:
        ...
        str_repr = str(result)
        assert str_repr == "WhoAmI: DN: cn=REDACTED_LDAP_BIND_PASSWORD,dc=example,dc=com"
...
class TestWhoAmIExtension:
    ...
    def test_decode_response_value_dn(self) -> None:
        ...
        dn_bytes = b"dn:cn=REDACTED_LDAP_BIND_PASSWORD,dc=example,dc=com"
        result = WhoAmIExtension.decode_response_value(None, dn_bytes)

        assert isinstance(result, WhoAmIResult)
        assert result.authorization_identity == "dn:cn=REDACTED_LDAP_BIND_PASSWORD,dc=example,dc=com"
        assert result.identity_type == IdentityType.DN
        assert result.get_dn() == "cn=REDACTED_LDAP_BIND_PASSWORD,dc=example,dc=com"
...
class TestIntegrationScenarios:
    ...
    def test_extension_request_response_cycle(self) -> None:
        ...
        server_response = b"dn:cn=REDACTED_LDAP_BIND_PASSWORD,ou=people,dc=company,dc=org"
...
class TestSecurityValidation:
    ...
    def test_identity_injection_protection(self) -> None:
        ...
        malicious_identities = [
            "dn:cn=REDACTED_LDAP_BIND_PASSWORD,dc=example,dc=com\x00extra",  # Null byte injection
