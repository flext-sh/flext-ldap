...
class TestFlextLdapAclConvertersOracleConverter:
    ...
    def test_to_openldap_success(self) -> None:
        ...
        assert result.data is not None
        assert (
            "Converted Oracle ACL to OpenLDAP format: GRANT READ ON ou=people,dc=example,dc=com TO cn=REDACTED_LDAP_BIND_PASSWORD,dc=example,dc=com"
            in result.data
        )
    ...
    def test_to_microsoft_ad_success(self) -> None:
        ...
        assert result.data is not None
        assert (
            "Converted Oracle ACL to Microsoft AD format: GRANT READ ON ou=people,dc=example,dc=com TO cn=REDACTED_LDAP_BIND_PASSWORD,dc=example,dc=com"
            in result.data
        )
