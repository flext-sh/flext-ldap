...
class TestFlextLdapModels:
    ...
    def test_create_permission_entry_success(self, acl_models: FlextLdapModels) -> None:
        ...
        assert "read" in permissions.granted_permissions
        assert "write" in permissions.granted_permissions
...
class TestFlextLdapAclParsersAciParser:
    ...
    def test_parse_groupdn_subject(self) -> None:
        ...
        assert result.unwrap().subject.subject_type == "group"
        assert (
            result.unwrap().subject.subject_dn == "ldap:///cn=REDACTED_LDAP_BIND_PASSWORDs,dc=example,dc=com"
        )
