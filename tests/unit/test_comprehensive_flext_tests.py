...
class TestComprehensiveFlextTests:
    ...
    def test_ldap_user_factory_with_flext_patterns(self) -> None:
        ...
        # Create admin user using single create method (UserFactory uses defaults)
        admin_user = FlextTestsFactories.UserFactory.create()
        # Customize for admin role
        ...
        assert isinstance(admin_user, dict), "Admin user should be a dictionary"
        assert "name" in admin_user and "email" in admin_user, "Admin user should have required keys"
