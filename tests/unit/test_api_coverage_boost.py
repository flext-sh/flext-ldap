...
class TestFlextLDAPApiCoverageBoost:
    ...
    def test_user_factory_integration_comprehensive(self) -> None:
        ...
        REDACTED_LDAP_BIND_PASSWORD = AdminUserFactory.create_object(
            email="REDACTED_LDAP_BIND_PASSWORD@example.com",
            age=40,
        )

        assert REDACTED_LDAP_BIND_PASSWORD.email == "REDACTED_LDAP_BIND_PASSWORD@example.com"
        assert REDACTED_LDAP_BIND_PASSWORD.age == 40
    ...
    async def test_api_connection_comprehensive(self) -> None:
        ...
        connection_tests = [
            ...
            ("ldap://localhost:3390", "cn=REDACTED_LDAP_BIND_PASSWORD,dc=test,dc=local", "password123"),
            ("ldap://localhost:389", "cn=REDACTED_LDAP_BIND_PASSWORD,dc=example,dc=com", "REDACTED_LDAP_BIND_PASSWORD"),
            ("ldaps://localhost:636", "cn=manager,dc=secure,dc=org", "secret"),
            # Invalid parameters (should fail with validation)
            ("", "cn=REDACTED_LDAP_BIND_PASSWORD,dc=test,dc=local", "password123"),  # Empty server
            ("ldap://localhost:3390", "", "password123"),  # Empty bind_dn
            (
                "ldap://localhost:3390",
                "cn=REDACTED_LDAP_BIND_PASSWORD,dc=test,dc=local",
                "",
            ),  # Empty password
            ("invalid-uri", "cn=REDACTED_LDAP_BIND_PASSWORD,dc=test,dc=local", "password123"),  # Invalid URI
        ]
