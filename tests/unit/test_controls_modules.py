        ...
        class MockPasswordPolicyControl:
            ...
            def check_password_policy(self, policy_info: dict[str, Any]) -> dict[str, Any]:
                ...
                if policy_info.get("account_locked"):
                    ...
                    recommendations.append("Contact REDACTED_LDAP_BIND_PASSWORDistrator to unlock account")
...
class TestProxyAuthControl:
    ...
    def test_proxy_auth_import(self) -> None:
        ...
        try:
            ...
            control = ProxyAuthControl(authorization_id="dn:cn=REDACTED_LDAP_BIND_PASSWORD,dc=example,dc=com")
            assert control is not None
    ...
    def _test_proxy_auth_mock(self) -> None:
        ...
        class MockProxyAuthControl:
            ...
            def check_proxy_permissions(self, user_dn: str) -> dict[str, Any]:
                ...
                REDACTED_LDAP_BIND_PASSWORD_users = [
                    ...
                    "cn=proxy-REDACTED_LDAP_BIND_PASSWORD,dc=example,dc=com",
                ]

                has_permission = user_dn in REDACTED_LDAP_BIND_PASSWORD_users
                ...
                return {
                    ...
                    "user_dn": user_dn,
                    "target_dn": target_dn,
                    "permission_type": "REDACTED_LDAP_BIND_PASSWORD" if has_permission else "none",
                    "can_proxy": has_permission,
                    "reason": "User in REDACTED_LDAP_BIND_PASSWORD list" if has_permission else "User not authorized for proxy operations",
                }
        ...
        permission_check = proxy_control.check_proxy_permissions("cn=REDACTED_LDAP_BIND_PASSWORD,dc=example,dc=com")
        assert permission_check["has_permission"] is True
        assert permission_check["can_proxy"] is True

        non_REDACTED_LDAP_BIND_PASSWORD_check = proxy_control.check_proxy_permissions("cn=regularuser,dc=example,dc=com")
        assert non_REDACTED_LDAP_BIND_PASSWORD_check["has_permission"] is False
        assert non_REDACTED_LDAP_BIND_PASSWORD_check["can_proxy"] is False
