...
# Test real clients functionality
from flext_ldap.clients import FlextLdapClient, SCOPE_MAP, LDAP_SUBORDINATES
...
class TestRealClientConstants:
    ...
    def test_ldap_subordinates_constant_is_valid(self) -> None:
        """Test LDAP_SUBORDINATES constant is properly defined."""
        assert LDAP_SUBORDINATES is not None
        
        # Should be an ldap3 constant or fallback to SUBTREE
        import ldap3
        if hasattr(ldap3, 'SUBORDINATES'):
            assert LDAP_SUBORDINATES == ldap3.SUBORDINATES
        else:
            assert LDAP_SUBORDINATES == ldap3.SUBTREE
    ...
    def test_scope_map_values_are_valid_ldap3_constants(self) -> None:
        ...
        for scope_name, scope_value in SCOPE_MAP.items():
            assert scope_value in valid_scopes, f"Invalid scope value for {scope_name}: {scope_value}"
...
class TestRealClientIntegration:
    ...
    def test_client_uses_flext_logger(self) -> None:
        ...
        logger = getattr(clients_module, 'logger')
...
class TestRealClientUtilities:
    ...
    def test_client_uses_flext_utilities(self) -> None:
        ...
        utilities = getattr(clients_module, 'FlextLdapUtilities')
    ...
    def test_client_constants_are_properly_imported(self) -> None:
        ...
        for import_name in expected_imports:
            assert hasattr(clients_module, import_name), f"Missing import: {import_name}"
...
class TestRealClientArchitecture:
    ...
    def test_client_follows_solid_principles(self) -> None:
        ...
        for method_name in unrelated_methods:
            assert not hasattr(client, method_name), f"Client has unrelated method: {method_name}"
