...
class TestGroupMembershipValidation:
    ...
    @pytest.mark.unit
    def test_REDACTED_LDAP_BIND_PASSWORD_group_requires_email(self) -> None:
        """Test that REDACTED_LDAP_BIND_PASSWORD group members must have email."""
        user = FlextLdifModels.Entry.create(
            dn="cn=john.doe,ou=users,dc=example,dc=com",
            attributes={"cn": ["john.doe"]},
        ).unwrap()

        REDACTED_LDAP_BIND_PASSWORD_group = FlextLdifModels.Entry.create(
    ...
    def test_REDACTED_LDAP_BIND_PASSWORD_group_with_email_succeeds(self) -> None:
        """Test REDACTED_LDAP_BIND_PASSWORD group membership succeeds with email."""
        user = FlextLdifModels.Entry.create(
            dn="cn=john.doe,ou=users,dc=example,dc=com",
            attributes={
                "cn": ["john.doe"],
    ...
    def test_regular_group_no_email_required(self) -> None:
        """Test regular (non-REDACTED_LDAP_BIND_PASSWORD) groups don't require email."""
        user = FlextLdifModels.Entry.create(
            dn="cn=john.doe,ou=users,dc=example,dc=com",
            attributes={"cn": ["john.doe"]},
        ).unwrap()
