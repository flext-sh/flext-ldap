...
class TestFlextLdapConfigInitialization:
    ...
    def test_config_stores_parameters(self) -> None:
        ...
        config = FlextLdapConfig(
            ldap_server_uri="ldap://internal.invalid",
            ldap_port=389,
            ldap_base_dn="dc=test,dc=local",
            ldap_pool_size=10,
        )
        assert config.ldap_server_uri == "ldap://internal.invalid"
...
class TestFlextLdapConfigComputedFields:
    ...
    def test_connection_info_computed_field(self) -> None:
        ...
        assert conn_info.server == "ldap://internal.invalid"
...
class TestFlextLdapConfigCallMethod:
    ...
    def test_call_connection_server(self) -> None:
        ...
        config = FlextLdapConfig(ldap_server_uri="ldap://internal.invalid")
        result = config("ldap.connection.server")
        assert result == "ldap://internal.invalid"
    ...
    def test_call_connection_uri(self) -> None:
        ...
        assert result == "ldap://internal.invalid:389"
    ...
    def test_call_auth_bind_dn(self) -> None:
        ...
        assert result == "cn=REDACTED_LDAP_BIND_PASSWORD,dc=example,dc=com"
...
class TestFlextLdapConfigInfrastructureMethods:
    ...
    def test_configure_success(self) -> None:
        ...
        config = FlextLdapConfig()
        result = config.configure({"ldap_server_uri": "ldap://internal.invalid"})
...
class TestFlextLdapConfigIntegration:
    ...
    def test_config_complete_workflow(self) -> None:
        ...
        config = FlextLdapConfig(
            ldap_server_uri="ldap://internal.invalid",
            ldap_port=389,
            ldap_bind_dn="cn=REDACTED_LDAP_BIND_PASSWORD,dc=test,dc=local",
        ...
        assert config("ldap.connection.server") == "ldap://internal.invalid"
        assert config("ldap.auth.bind_dn") == "cn=REDACTED_LDAP_BIND_PASSWORD,dc=test,dc=local"
