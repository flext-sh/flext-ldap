...
class TestFlextLdapModelsConnectionConfig:
    ...
    def test_connection_config_basic_creation(self) -> None:
        ...
        assert config.bind_dn == "cn=admin,dc=flext,dc=local"
        assert config.bind_password == "admin123"
    ...
    def test_full_search_workflow(self) -> None:
        ...
        connect_result = client.connect(
            FlextLdapModels.ConnectionRequest(
                ...
                bind_dn=config.bind_dn or "cn=admin,dc=flext,dc=local",
                password=config.bind_password or "admin123",
            )
        )
        assert connect_result.is_success is True
