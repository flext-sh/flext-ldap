...
class TestFlextTestsIntegration:
    ...
    def test_admin_user_factory_integration(self) -> None:
        ...
        admin_users = [TestDataFactory.create_user() for _ in range(3)]

        for admin in admin_users:
            # Verify admin has required attributes
            assert "email" in admin
            admin_name = admin.get("name", "Admin User")
            admin_email = admin.get("email", "admin@example.com")

            # Create admin LDAP request
            admin_request = FlextLDAPEntities.CreateUserRequest(
                dn=f"cn={admin_name.replace(' ', '.')},ou=admins,dc=example,dc=com",
                uid=admin_name.replace(" ", ".").lower(),
                cn=admin_name,
                mail=admin_email,
            )
