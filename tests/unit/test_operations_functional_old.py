...
class TestFlextLdapClientFunctional:
    ...
    def test_flext_ldap_operations_import_and_structure(self) -> None:
        ...
        available_attrs = [
            attr for attr in dir(FlextLdapClient) if not attr.startswith("_")
        ]
    ...
    def test_operations_module_loads_without_errors(self) -> None:
        ...
        # FlextLdapClient is imported directly, no need to check ops_module

        # Check that FlextLdapClient has expected functionality
        client_attrs = [
            attr for attr in dir(FlextLdapClient) if not attr.startswith("_")
        ]
    ...
    def test_operations_class_instantiation(self) -> None:
        """Test FlextLdapClient class can be instantiated if it has constructor."""
        # Try to understand the operations structure
        ops_attrs = [
            attr
            for attr in dir(FlextLdapClient)
            if not attr.startswith("_")
            and not callable(getattr(FlextLdapClient, attr, None))
        ]
        ...
        class_methods = [
            attr
            for attr in dir(FlextLdapClient)
            if callable(getattr(FlextLdapClient, attr, None))
...
class TestFlextLdapClientIntegration:
...
class TestLDAPCommandProcessor:
    """Test FlextLdapClient._CommandProcessor functionality - expanding coverage."""
...
class TestLDAPAttributeProcessor:
    ...
    def test_group_attribute_extractor_process_data(self) -> None:
        ...
        ldap_group_attributes = {
            "cn": ["REDACTED_LDAP_BIND_PASSWORD_group"],
...
class TestLdapUserModel:
    """Test LdapUser model functionality."""
...
class TestFlextLdapClientMainClass:
    """Test main FlextLdapClient class functionality."""

    def test_operations_main_class_structure(self) -> None:
        """Test FlextLdapClient main class has expected structure."""
        # This should cover more of the main class functionality
        ops_attrs = [attr for attr in dir(FlextLdapClient) if not attr.startswith("_")]
        ...
        assert hasattr(FlextLdapClient, "__name__")
        assert "Operations" in FlextLdapClient.__name__


class TestFlextLdapClientPerformance:
    """Test operations module performance characteristics."""

    def test_operations_import_performance(self) -> None:
        """Test that flext_ldap modules import efficiently."""
        start_time = time.time()

        # Re-import to test performance
        importlib.reload(sys.modules["flext_ldap.clients"])
    ...
    def test_operations_memory_efficiency(self) -> None:
        """Test that flext_ldap modules use memory efficiently."""
        # Check module size in memory
        if "flext_ldap.clients" in sys.modules:
            clients_module = sys.modules["flext_ldap.clients"]
            # Module should exist and be loadable
            assert clients_module is not None

            # Basic memory efficiency check
            module_attrs = dir(clients_module)
...
class TestConnectionOperations:
    ...
    def test_connection_operations_creation(self) -> None:
        """Test FlextLdapClient can be created."""
        operations_instance = FlextLdapClient()
        assert operations_instance is not None
    ...
    def test_connection_operations_methods(self) -> None:
        ...
        connection_methods = [
            attr
            for attr in dir(operations_instance)
            if not attr.startswith("_") and callable(getattr(operations_instance, attr))
        ]
    ...
    @pytest.mark.asyncio
    async def test_operations_connect_and_bind(self) -> None:
        """Test connect and bind methods."""
        operations = FlextLdapClient()

        # Test connection creation (will fail without server, but tests method exists)
        try:
            result = await operations.connect(
    ...
    def test_unbind_method(self) -> None:
        ...
        assert hasattr(operations, "unbind")
        assert callable(operations.unbind)
...
class TestSearchOperations:
    ...
    def test_search_operations_methods(self) -> None:
        """Test FlextLdapClient has search methods."""
        operations = FlextLdapClient()

        # Test search methods are available
        assert hasattr(operations, "search")
        assert hasattr(operations, "search_with_request")
        assert callable(operations.search)
        assert callable(operations.search_with_request)
...
class TestLDAPModificationOperations:
    ...
    def test_entry_validation_methods(self) -> None:
        ...
        validation_methods = [
            method
            for method in dir(FlextLdapValidations)
            if method.startswith("validate_")
            and callable(getattr(FlextLdapValidations, method))
        ]
        ...
        for method_name in validation_methods[
            ...
            method = getattr(FlextLdapValidations, method_name)
...
class TestConnectionManagement:
    ...
    def test_connection_error_handling(self) -> None:
        ...
        try:
            ...
            # Use FlextLdapValidations for validation
            from flext_ldap.validations import FlextLdapValidations
...
class TestGroupOperations:
    ...
    def test_group_operations_creation(self) -> None:
        """Test FlextLdapClient can be created for group operations."""
        operations = FlextLdapClient()
        assert operations is not None

    def test_group_operations_methods(self) -> None:
        """Test FlextLdapClient has expected methods for group operations."""
        operations = FlextLdapClient()

        # Test that specific group operations are available
        assert hasattr(operations, "create_group")
        assert hasattr(operations, "get_group")
        assert hasattr(operations, "update_group")
        assert hasattr(operations, "delete_group")
        assert hasattr(operations, "add_member")
        assert hasattr(operations, "remove_member")
        assert hasattr(operations, "get_members")
...
class TestOperationsService:
    """Test OperationsService functionality using FlextLdapClient."""
    ...
    def test_operations_service_methods(self) -> None:
        """Test FlextLdapClient has expected service methods."""
        operations = FlextLdapClient()

        # Check for service-related methods
        assert hasattr(operations, "connect")
        assert hasattr(operations, "bind")
        assert hasattr(operations, "unbind")
        assert hasattr(operations, "is_connected")
        assert hasattr(operations, "test_connection")


class TestMainOperationsInstanceMethods:
    """Test main FlextLdapClient instance methods - targeting missing coverage."""
    ...
    def test_search_property(self) -> None:
        ...
        assert hasattr(operations, "search")
        assert hasattr(operations, "search_with_request")
        assert hasattr(operations, "search_users")
        assert hasattr(operations, "search_groups")

    def test_entries_property(self) -> None:
        """Test entry operations methods are available."""
        operations = FlextLdapClient()
        # Test that entry operation methods exist
        assert hasattr(operations, "add")
        assert hasattr(operations, "modify")
        assert hasattr(operations, "delete")
        assert hasattr(operations, "add_entry")
        assert hasattr(operations, "modify_entry")

    def test_users_property(self) -> None:
        """Test user operations functionality."""
        operations = FlextLdapClient()
        # Test that user operation methods exist
        assert hasattr(operations, "create_user")
        assert hasattr(operations, "get_user")
        assert hasattr(operations, "update_user")
        assert hasattr(operations, "delete_user")

    def test_groups_property(self) -> None:
        """Test group operations functionality."""
        operations = FlextLdapClient()
        # Test that group operation methods exist
        assert hasattr(operations, "create_group")
        assert hasattr(operations, "get_group")
        assert hasattr(operations, "update_group")
        assert hasattr(operations, "delete_group")
    ...
    def test_generate_id_method(self) -> None:
        """Test session_id method returns valid ID."""
        operations = FlextLdapClient()
        ...
        assert re.match(r"^[a-zA-Z0-9_-]+$", session_id) is not None
...
class TestConnectionOperationsDetailed:
    ...
    def test_get_connection_info_method(self) -> None:
        ...
        assert hasattr(operations_instance, "is_connected")
        assert hasattr(operations_instance, "test_connection")
        assert hasattr(operations_instance, "get_connection_string")
    ...
    def test_list_active_connections_method(self) -> None:
        ...
        assert hasattr(operations_instance, "connect")
        assert hasattr(operations_instance, "bind")
        assert hasattr(operations_instance, "unbind")
        assert hasattr(operations_instance, "reconnect")

        # Test that methods are callable
        assert callable(operations_instance.connect)
        assert callable(operations_instance.bind)
        assert callable(operations_instance.unbind)
...
class TestLDAPEntryProcessing:
    ...
    def test_attribute_extraction_scenarios(self) -> None:
        ...
        # Test that FlextLdapClient has methods for attribute processing
        operations = FlextLdapClient()

        # Test that methods exist for processing attributes
        assert hasattr(operations, "add")
        assert hasattr(operations, "modify")
        assert hasattr(operations, "add_entry")
        assert hasattr(operations, "modify_entry")
        ...
        assert callable(operations.modify)
        assert callable(operations.add_entry)
        assert callable(operations.modify_entry)

    def test_group_attribute_extraction_scenarios(self) -> None:
        """Test group attribute extraction in various scenarios."""
        operations = FlextLdapClient()

        # Test that group methods exist
        assert hasattr(operations, "create_group")
        assert hasattr(operations, "get_group")
        assert hasattr(operations, "update_group")
        assert hasattr(operations, "delete_group")
...
class TestCommandObjectExecution:
    ...
    def test_search_command_execution_paths(self) -> None:
        """Test search methods are available in FlextLdapClient."""
        # Use the correct unified class pattern
        operations = FlextLdapClient()
        ...
        assert callable(operations.search)
        assert callable(operations.search_with_request)
        assert callable(operations.search_users)
        assert callable(operations.search_groups)

    def test_membership_command_execution_paths(self) -> None:
        """Test group membership operations are available."""
        # Use the correct unified class pattern
        operations = FlextLdapClient()

        # Test that group membership methods exist
        assert hasattr(operations, "add_member")
        assert hasattr(operations, "remove_member")
        assert hasattr(operations, "get_members")
        assert hasattr(operations, "search_groups")
        ...
        # Test that methods are callable
        assert callable(operations.add_member)
        assert callable(operations.remove_member)
        assert callable(operations.get_members)
        assert callable(operations.search_groups)
    ...
    def test_connection_operations_real_execution(self) -> None:
        ...
        assert callable(operations_instance.bind)
        assert callable(operations_instance.unbind)
        assert callable(operations_instance.is_connected)
        assert callable(operations_instance.test_connection)
...
class TestOperationsInternalMethods:
    ...
    def test_validation_methods_with_edge_cases(self) -> None:
        ...
        for _uri in uri_test_cases:
            # Test DN validation instead since validate_ldap_uri doesn't exist
    ...
    async def test_cleanup_connection_execution(self) -> None:
        ...
        for _conn_id in test_connection_ids:
            # Test that connection methods exist
            assert hasattr(operations, "connect")
            assert hasattr(operations, "bind")
            assert hasattr(operations, "unbind")
            # Test that methods are callable
            assert callable(operations.connect)
            assert callable(operations.bind)
            assert callable(operations.unbind)
...
class TestDetailedAttributeExtraction:
    ...
    def test_user_extractor_attribute_methods(self) -> None:
        """Test that FlextLdapClient has user-related methods."""
        operations = FlextLdapClient()

        # Test that user methods exist
        assert hasattr(operations, "create_user")
        assert hasattr(operations, "get_user")
        assert hasattr(operations, "update_user")
        assert hasattr(operations, "delete_user")
        ...
        # Test that methods are callable
        assert callable(operations.create_user)
        assert callable(operations.get_user)
        assert callable(operations.update_user)
        assert callable(operations.delete_user)
        assert callable(operations.search_users)

    def test_group_extractor_member_extraction(self) -> None:
        """Test that FlextLdapClient has group-related methods."""
        operations = FlextLdapClient()
        ...
        # Test that methods are callable
        assert callable(operations.create_group)
        assert callable(operations.get_group)
        assert callable(operations.update_group)
        assert callable(operations.delete_group)
        assert callable(operations.add_member)
        assert callable(operations.remove_member)
        assert callable(operations.get_members)
        assert callable(operations.search_groups)
    ...
    def test_attribute_extraction_error_paths(self) -> None:
        """Test that FlextLdapClient has error handling capabilities."""
        operations = FlextLdapClient()
...
class TestAdvancedExecutionPaths:
    ...
    def test_connection_operations_close_connection(self) -> None:
        ...
        assert hasattr(operations_instance, "is_connected")
        assert hasattr(operations_instance, "test_connection")
        assert hasattr(operations_instance, "close_connection")
        ...
        assert callable(operations_instance.is_connected)
        assert callable(operations_instance.test_connection)
        assert callable(operations_instance.close_connection)
    ...
    def test_flext_ldap_operations_main_init(self) -> None:
        """Test FlextLdapClient.__init__ method execution."""
        # Test various initialization scenarios
        ops1 = FlextLdapClient()
        assert ops1 is not None

        ops2 = FlextLdapClient()
...
class TestAttributeExtractionAdvanced:
    ...
    def test_extract_optional_string_attribute_comprehensive(self) -> None:
        ...
        extractor = FlextLdapClient()._extractors
        ...
        if hasattr(operations, "ConnectionOperations"):
            ...
            if hasattr(conn_ops, "create_connection"):
                result = await conn_ops.create_connection(
                    ...
                    bind_dn="cn=REDACTED_LDAP_BIND_PASSWORD,dc=example,dc=com",
                    _bind_password="REDACTED_LDAP_BIND_PASSWORD123",
                    timeout_seconds=30,
                )
                assert hasattr(result, "is_success")
