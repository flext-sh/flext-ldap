...
class TestFlextLdapDistinguishedName:
    ...
    def test_valid_dn_creation(self) -> None:
        ...
        dn = FlextLdapDistinguishedName(value="cn=REDACTED_LDAP_BIND_PASSWORD,dc=example,dc=org")
        assert dn.value == "cn=REDACTED_LDAP_BIND_PASSWORD,dc=example,dc=org"
        assert str(dn) == "cn=REDACTED_LDAP_BIND_PASSWORD,dc=example,dc=org"
    ...
    def test_dn_validation_invalid_component(self) -> None:
        ...
        with pytest.raises(ValidationError):
            FlextLdapDistinguishedName(value="cn=REDACTED_LDAP_BIND_PASSWORD,invalid,dc=org")

    def test_dn_validation_empty_attribute_name(self) -> None:
        """Test DN validation with empty attribute name."""
        with pytest.raises(ValidationError):
            FlextLdapDistinguishedName(value="=REDACTED_LDAP_BIND_PASSWORD,dc=example,dc=org")
    ...
    def test_get_rdn(self) -> None:
        ...
        dn = FlextLdapDistinguishedName(value="cn=REDACTED_LDAP_BIND_PASSWORD,dc=example,dc=org")
        assert dn.get_rdn() == "cn=REDACTED_LDAP_BIND_PASSWORD"
    ...
    def test_get_components(self) -> None:
        ...
        dn = FlextLdapDistinguishedName(value="cn=REDACTED_LDAP_BIND_PASSWORD,ou=users,dc=example,dc=org")
        components = dn.get_components()
        expected = ["cn=REDACTED_LDAP_BIND_PASSWORD", "ou=users", "dc=example", "dc=org"]
        assert components == expected

    def test_get_components_with_spaces(self) -> None:
        """Test getting DN components with spaces."""
        dn = FlextLdapDistinguishedName(value="cn=REDACTED_LDAP_BIND_PASSWORD , ou=users , dc=example")
        components = dn.get_components()
        expected = ["cn=REDACTED_LDAP_BIND_PASSWORD", "ou=users", "dc=example"]
        assert components == expected

    def test_is_child_of_true(self) -> None:
        """Test is_child_of returns True for child relationship."""
        child = FlextLdapDistinguishedName(value="cn=REDACTED_LDAP_BIND_PASSWORD,dc=example,dc=org")
    ...
    def test_is_child_of_false(self) -> None:
        ...
        dn1 = FlextLdapDistinguishedName(value="cn=REDACTED_LDAP_BIND_PASSWORD,dc=example,dc=org")
...
class TestFlextLdapFilterValue:
    ...
    def test_valid_filter_creation(self) -> None:
        ...
        filter_val = FlextLdapFilterValue(value="(cn=REDACTED_LDAP_BIND_PASSWORD)")
        assert filter_val.value == "(cn=REDACTED_LDAP_BIND_PASSWORD)"
        assert str(filter_val) == "(cn=REDACTED_LDAP_BIND_PASSWORD)"

    def test_complex_filter_creation(self) -> None:
        """Test creating complex LDAP filter."""
        complex_filter = "(&(cn=REDACTED_LDAP_BIND_PASSWORD)(objectClass=person))"
    ...
    def test_filter_validation_no_parentheses(self) -> None:
        ...
        with pytest.raises(ValidationError):
            FlextLdapFilterValue(value="cn=REDACTED_LDAP_BIND_PASSWORD")
    ...
    def test_equals_filter_creation(self) -> None:
        ...
        filter_val = FlextLdapFilterValue.equals("cn", "REDACTED_LDAP_BIND_PASSWORD")
        assert filter_val.value == "(cn=REDACTED_LDAP_BIND_PASSWORD)"
    ...
    def test_and_filters_single(self) -> None:
        ...
        filter1 = FlextLdapFilterValue(value="(cn=REDACTED_LDAP_BIND_PASSWORD)")
        result = FlextLdapFilterValue.and_filters(filter1)
        assert result.value == "(cn=REDACTED_LDAP_BIND_PASSWORD)"
    ...
    def test_and_filters_multiple(self) -> None:
        ...
        assert result.value == "(&(cn=REDACTED_LDAP_BIND_PASSWORD)(objectClass=person))"
    ...
    def test_or_filters_multiple(self) -> None:
        ...
        assert result.value == "(|(cn=REDACTED_LDAP_BIND_PASSWORD)(cn=user))"
