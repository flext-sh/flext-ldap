...
class TestBindableProtocol:
    ...
    async def test_simple_bind_success(self, bindable: TestBindable) -> None:
        ...
        result = await bindable.bind("cn=REDACTED_LDAP_BIND_PASSWORD,dc=example,dc=com", "password")
        assert result is True

    async def test_simple_bind_failure(self, bindable: TestBindable) -> None:
        """Test failed simple bind."""
        result = await bindable.bind("cn=REDACTED_LDAP_BIND_PASSWORD,dc=example,dc=com", "wrong")
        assert result is False
    ...
    async def test_unbind(self, bindable: TestBindable) -> None:
        ...
        await bindable.bind("cn=REDACTED_LDAP_BIND_PASSWORD,dc=example,dc=com", "password")
