...
class TestFlextLdapDistinguishedNameSpecification:
    ...
    def test_is_satisfied_by_invalid_component(self) -> None:
        ...
        invalid_dns = [
            "cn=admin,invalid,dc=org",  # Component without =
            "cn=admin,=value,dc=org",  # Empty attribute name
            "cn=admin,attr=,dc=org",  # Empty attribute value
            "cn=,dc=example,dc=org",  # Empty CN value
            "=admin,dc=example,dc=org",  # Empty attribute name
        ]
...
class TestFlextLdapFilterSpecification:
    ...
    def test_is_satisfied_by_no_parentheses(self) -> None:
        ...
        if spec.is_satisfied_by("cn=admin"):
            msg: str = f"Expected False, got {spec.is_satisfied_by('cn=admin')}"
            raise AssertionError(
                msg,
            )
    ...
    def test_is_satisfied_by_unbalanced_parentheses(self) -> None:
        ...
        unbalanced_filters = [
            "((cn=admin)",  # Extra opening
            "(cn=admin))",  # Extra closing
            "((cn=admin)(",  # Multiple imbalances
        ]
    ...
    def test_is_satisfied_by_complex_balanced_filter(self) -> None:
        ...
        complex_filter = (
            "(&(|(cn=admin)(cn=user))(objectClass=person)(!(accountDisabled=true)))"
        )
