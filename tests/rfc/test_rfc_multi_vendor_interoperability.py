    ...
    async def test_active_directory_authentication_methods(self) -> None:
        ...
        ad_auth_scenarios = [
            ...
            },
            {
                ...
                "config": {
                    "server": "ldap://ad.example.com:389",
                    "auth_dn": "REDACTED_LDAP_BIND_PASSWORDistrator@example.com",  # UPN format
                    "auth_password": "password",
                    "auth_method": "simple",
                },
            },
            {
                ...
                "config": {
                    "server": "ldap://ad.example.com:389",
                    "auth_dn": "EXAMPLE\\REDACTED_LDAP_BIND_PASSWORDistrator",  # SAM format
                    "auth_password": "password",
                    "auth_method": "simple",
                },
            },
            {
                ...
                "config": {
                    ...
                    "kerberos_principal": "REDACTED_LDAP_BIND_PASSWORDistrator@EXAMPLE.COM",
                },
            },
        ]
    ...
    async def test_openldap_schema_compatibility(self) -> None:
        ...
        with patch("ldap3.Connection") as mock_conn_class:
            ...
            openldap_config = LDAPConfig(
                ...
                auth_dn="cn=REDACTED_LDAP_BIND_PASSWORD,dc=example,dc=com",  # OpenLDAP REDACTED_LDAP_BIND_PASSWORD DN format
                auth_password="password",
    ...
    async def test_389ds_schema_compatibility(self) -> None:
        ...
        with patch("ldap3.Connection") as mock_conn_class:
            ...
            ds389_config = LDAPConfig(
                ...
                auth_dn="cn=Directory Manager",  # 389 DS REDACTED_LDAP_BIND_PASSWORD DN
                auth_password="password",
                server_type="389ds",
...
class TestCrossVendorCompatibility:
        ...
        with patch("ldap3.Connection") as mock_conn_class:
            for vendor, schema_mapping in vendor_schema_mappings.items():
                ...
                config = LDAPConfig(
                    ...
                    auth_dn=self._get_vendor_REDACTED_LDAP_BIND_PASSWORD_dn(vendor),
                    auth_password="password",
                    server_type=vendor,
    ...
    def _get_vendor_REDACTED_LDAP_BIND_PASSWORD_dn(self, vendor: str) -> str:
        """Get vendor-specific REDACTED_LDAP_BIND_PASSWORD DN."""
        if vendor == "active_directory":
            return "CN=Administrator,CN=Users,DC=example,DC=com"
        if vendor == "openldap":
            return "cn=REDACTED_LDAP_BIND_PASSWORD,dc=example,dc=com"
        if vendor == "389ds":
