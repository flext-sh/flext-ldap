...
class TestFlextLdapInfrastructureClient:
    ...
    async def test_connect_success(self) -> None:
        ...
        with (
            ...
            if result.data != "ldap://test.com:cn=admin":
                raise AssertionError(f"Expected {"ldap://test.com:cn=admin"}, got {result.data}")
            if "ldap://test.com:cn=admin" not in adapter._connections:
                raise AssertionError(f"Expected {"ldap://test.com:cn=admin"} in {adapter._connections}")
    ...
    async def test_connect_with_ssl(self) -> None:
        ...
        with (
            ...
            assert result.is_success
            if result.data != "ldaps://test.com:cn=admin":
                raise AssertionError(f"Expected {"ldaps://test.com:cn=admin"}, got {result.data}")
    ...
    def test_get_connection_info_success(self) -> None:
        ...
        if info["user"] != "cn=admin,dc=test":
            raise AssertionError(f"Expected {"cn=admin,dc=test"}, got {info["user"]}")
