        ...
        with (
            ...
            if result.data != "ldap://test.com:cn=admin":
                raise AssertionError(
                    f"Expected {'ldap://test.com:cn=admin'}, got {result.data}"
                )
            if "ldap://test.com:cn=admin" not in adapter._connections:
                raise AssertionError(
                    f"Expected {'ldap://test.com:cn=admin'} in {adapter._connections}"
                )
    ...
    async def test_connect_with_ssl(self) -> None:
        ...
        with (
            ...
            if result.data != "ldaps://test.com:cn=admin":
                raise AssertionError(
                    f"Expected {'ldaps://test.com:cn=admin'}, got {result.data}"
                )
