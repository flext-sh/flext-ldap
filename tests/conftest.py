...
class TestUser(BaseEntity):
    ...
    def can_be_deleted(self) -> bool:
        """Business rule: REDACTED_LDAP_BIND_PASSWORDs and active users with important data cannot be deleted."""
...
@pytest.fixture
def REDACTED_LDAP_BIND_PASSWORD_user(user_factory: Any) -> TestUser:
    """Standard REDACTED_LDAP_BIND_PASSWORD user for authorization tests."""
    return user_factory(username="REDACTED_LDAP_BIND_PASSWORD", role="REDACTED_LDAP_BIND_PASSWORD", age=35)
...
def user_strategy():
    ...
    return st.builds(
        ...
        role=st.sampled_from(["user", "REDACTED_LDAP_BIND_PASSWORD", "moderator", "guest"]),
    )
    ...
    def generate_user_batch(count: int, factory_func: Callable) -> list[TestUser]:
        ...
        return [
            factory_func(
                ...
                age=18 + (i % 60),
                is_active=i % 3 != 0,
                role="REDACTED_LDAP_BIND_PASSWORD" if i % 10 == 0 else "user",
            )
