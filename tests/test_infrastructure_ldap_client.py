...
class TestFlextLdapInfrastructureClient:
    ...
    def mock_connection(self) -> MagicMock:
        ...
        mock_conn.user = "cn=REDACTED_LDAP_BIND_PASSWORD,dc=example,dc=org"
    ...
    async def test_connect_success(
        ...
        assert result.success is True
        assert result.data == "ldap://localhost:389:cn=REDACTED_LDAP_BIND_PASSWORD,dc=example,dc=org"
    ...
    async def test_create_user_success(
        ...
        mock_conn_entity.bind_dn = "cn=REDACTED_LDAP_BIND_PASSWORD,dc=example,dc=org"

        # Create connection ID that method will use
        connection_id = "ldap://localhost:389:cn=REDACTED_LDAP_BIND_PASSWORD,dc=example,dc=org"
        ldap_client._connections[connection_id] = mock_connection
