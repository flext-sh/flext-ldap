...
class TestFlextLdapFilter:
    ...
    def test_filter_creation(self) -> None:
        ...
        filter_obj = FlextLdapFilter(filter_string="(cn=REDACTED_LDAP_BIND_PASSWORD)")
        assert filter_obj.filter_string == "(cn=REDACTED_LDAP_BIND_PASSWORD)"
        assert str(filter_obj) == "(cn=REDACTED_LDAP_BIND_PASSWORD)"

    def test_equals_filter(self) -> None:
        """Test creating equals filter."""
        filter_obj = FlextLdapFilter.equals("cn", "REDACTED_LDAP_BIND_PASSWORD")
        assert filter_obj.filter_string == "(cn=REDACTED_LDAP_BIND_PASSWORD)"
    ...
    def test_starts_with_filter(self) -> None:
        ...
        filter_obj = FlextLdapFilter.starts_with("cn", "REDACTED_LDAP_BIND_PASSWORD")
        assert filter_obj.filter_string == "(cn=REDACTED_LDAP_BIND_PASSWORD*)"

    def test_ends_with_filter(self) -> None:
        """Test creating ends-with filter."""
        filter_obj = FlextLdapFilter.ends_with("cn", "REDACTED_LDAP_BIND_PASSWORD")
        assert filter_obj.filter_string == "(cn=*REDACTED_LDAP_BIND_PASSWORD)"
    ...
    def test_not_equals_filter(self) -> None:
        ...
        filter_obj = FlextLdapFilter.not_equals("cn", "REDACTED_LDAP_BIND_PASSWORD")
        assert filter_obj.filter_string == "(!(cn=REDACTED_LDAP_BIND_PASSWORD))"

    def test_and_filter_single(self) -> None:
        """Test AND filter with single filter."""
        filter1 = FlextLdapFilter(filter_string="(cn=REDACTED_LDAP_BIND_PASSWORD)")
        result = FlextLdapFilter.and_filter(filter1)
        assert result.filter_string == "(&(cn=REDACTED_LDAP_BIND_PASSWORD))"
    ...
    def test_and_filter_multiple(self) -> None:
        ...
        assert result.filter_string == "(&(cn=REDACTED_LDAP_BIND_PASSWORD)(objectClass=person))"
    ...
    def test_or_filter_multiple(self) -> None:
        ...
        assert result.filter_string == "(|(cn=REDACTED_LDAP_BIND_PASSWORD)(cn=user))"
    ...
    def test_complex_filter_combination(self) -> None:
        ...
        name_filter = FlextLdapFilter.starts_with("cn", "REDACTED_LDAP_BIND_PASSWORD")
        mail_filter = FlextLdapFilter.present("mail")

        # ((objectClass=person) AND (cn=REDACTED_LDAP_BIND_PASSWORD*)) OR (mail=*)
        complex_filter = (person_filter & name_filter) | mail_filter
        expected = "(|(&(objectClass=person)(cn=REDACTED_LDAP_BIND_PASSWORD*))(mail=*))"
