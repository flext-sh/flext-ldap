...
class TestService:
    ...
    async def test_service_create_validation_failure(
        ...
        admin_user = TestUser(
            username="admin",  # Cannot be deleted = validation fails in create
            email="admin@example.com",
            age=30,
            is_active=False,  # Inactive admin
        )

        result = await user_service.create(admin_user)
    ...
    async def test_service_delete_business_rule_violation(
        ...
        # Create admin user (cannot be deleted per business rules)
        admin_user = TestUser(
            username="admin",
            email="admin@example.com",
            age=30,
        ...
        # Save admin
        await user_service._repository.save(admin_user)
...
class TestSpecification:
    ...
    def test_specification_or_composition(self) -> None:
        ...
        # Combined: admin OR senior
        admin_or_senior = admin_spec.or_(senior_spec)

        # Test cases
        admin_young = TestUser(username="admin", email="admin@example.com", age=30)
        ...
        assert admin_or_senior.is_satisfied_by(admin_young) is True
        assert admin_or_senior.is_satisfied_by(regular_senior) is True
        assert admin_or_senior.is_satisfied_by(regular_young) is False
    ...
    def test_specification_complex_composition(self) -> None:
        ...
        # Build complex specification: (adult AND active) OR admin
        adult_spec = MockSpecification(lambda user: user.age >= 18)
        active_spec = MockSpecification(lambda user: user.is_active)
        admin_spec = MockSpecification(lambda user: user.username == "admin")
