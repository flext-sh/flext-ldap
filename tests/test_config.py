...
class TestFlextLdapAuthConfig:
    ...
    def test_bind_dn_validation(self) -> None:
        ...
        if config.bind_dn != "cn=REDACTED_LDAP_BIND_PASSWORD,dc=example,dc=org"  # Should be stripped:
            raise AssertionError(f"Expected {"cn=REDACTED_LDAP_BIND_PASSWORD,dc=example,dc=org"  # Should be stripped}, got {config.bind_dn}")
