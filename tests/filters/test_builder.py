...
class TestConvenienceFunctions:
    ...
    def test_contains_function(self) -> None:
        ...
        expr = contains("cn", "REDACTED_LDAP_BIND_PASSWORD")

        assert isinstance(expr, FilterExpression)
        assert expr.filter_string == r"(cn=*REDACTED_LDAP_BIND_PASSWORD*)"
    ...
    def test_or_filters_function(self) -> None:
        ...
        filter1 = equals("cn", "REDACTED_LDAP_BIND_PASSWORD")
        filter2 = equals("cn", "REDACTED_LDAP_BIND_PASSWORDistrator")

        expr = or_filters(filter1, filter2)

        assert isinstance(expr, FilterExpression)
        assert expr.filter_string.startswith("(|")
        assert "(cn=REDACTED_LDAP_BIND_PASSWORD)" in expr.filter_string
        assert "(cn=REDACTED_LDAP_BIND_PASSWORDistrator)" in expr.filter_string
