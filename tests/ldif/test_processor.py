...
system with full CLAUDE.md compliance and algar-oud-mig integration validation.
...
Test Coverage:
    - LDIF parsing and validation with algar-oud-mig compatibility
...
class TestLDIFProcessorWorkspaceCompliance:
    ...
    def test_algar_oud_mig_integration_compatibility(self, workspace_coordination) -> None:
        ...
        # Test LDIF processor provides expected interface for algar-oud-mig
        config = LDIFProcessingConfig(
        ...
        # Test configuration matches algar-oud-mig requirements
    ...
    def test_algar_migration_performance_requirements(self) -> None:
        """Test LDIF processor meets algar-oud-mig performance requirements."""
        # Test LDIF processor can handle ALGAR migration performance requirements
        config = LDIFProcessingConfig(
            chunk_size=500,  # ALGAR LDAP optimal batch size
            max_entries=15000,  # Large ALGAR migration files
        ...
        # Validate configuration supports ALGAR requirements
        assert processor.config.chunk_size == 500
        assert processor.config.max_entries >= 15000
        assert processor.config.performance_monitoring is True

        # Test processor supports large file streaming (required for ALGAR)
        assert hasattr(processor, "stream_chunks")
...
class TestLDIFProcessingConfig:
    """Test LDIF processing configuration with CLAUDE.md compliance and algar-oud-mig compatibility."""
    ...
    def test_config_creation_with_defaults(self, workspace_coordination) -> None:
        ...
        # Validate configuration is suitable for algar-oud-mig integration
        assert config.chunk_size <= 1000  # Reasonable for large migrations
        assert config.max_entries >= 15000  # Supports ALGAR migration file sizes
